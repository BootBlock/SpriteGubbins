 ...
 *
 * **Raised from 2424 by the palette lock's ceiling** (issue #341), which refuses a lock past
 * `MAX_PALETTE_ENTRIES` colours and files the lock's entries on the OKLab lattice `mergeColors` already
 * walked, now shared as `oklabLattice`. Measured against `main` at `c302abf`, rebuilt from the same
 * lockfile, with the ceiling forced to zero in each tree so both figures are the ones
 * `assertPrecacheContract` receives: **2422.58 KiB** on `main` and **2424.35** here, a delta of
 * **1.77** that crossed a ceiling the base sat 1.42 under. No file was added to or removed from
 ...
 * bundled, for the tooltip clause that lists it. A shared chunk moved 0.47 KiB out of `database` and
 * 0.46 into `quantiseDials`, and `index` and `StudioTab` lost 0.03 between them.
 *
 * 2425 leaves **0.65 KiB**, the smallest whole figure over the build.
 */
