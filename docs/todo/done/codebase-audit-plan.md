...
> **Status:** ✅ COMPLETE — all ten phases and the wrap-up are done. The audit filed **fifty-nine
> issues**, #151–#177 and #179–#210, of which three are `priority: high`: #173, #193 and #194.
...
| 10 — Test suite and documentation | complete | 2026-08-31 | #197 #198 #199 #200 #201 #202 #203 #204 | **Mutation spot-checks are this phase's method, and they are what found both test defects.** Thirty-one mutations across seventeen suites, each applied in the worktree, run with `npx vitest run <file>`, and reverted before the next — twenty-six caught, two missed. Caught: a `duration-500` at a call site; a bracketed 10px font size; `text-ink` in a class string that also names `bg-accent-strong`; a wheel stop nudged 0.003 off L 0.76; `bg-spectrum` resized to 300%, breaking the one-image-width travel; a declaration dropped from the `[data-motion='reduced']` block; a `#ff0000` in a component; a straight double quote in a guidance string (caught by both `interface-punctuation.test.ts` and the tooltip suite); an option in full capitals and one opening lower case; a preset pinning a retired casing; a guidance entry cut to a stub; a `docs/todo/` banner removed; a character changed in `PROMPT_TEMPLATE`; the studio split moved to `lg:` and its column narrowed to 4/12; `max-w-sm` in a listed quantise panel; a sticky offset hard-coded to `9rem`; `zustand` removed from `optimizeDeps.include`; a `[SEC:]` naming no section; the icon ground moved off `foundry-900`; the only consumer of `TargetModel.description` reading `label` instead; a corpus sheet's expected pitch swapped; a 55-character `VOTE_METHOD_CHOICES` label; and `differenceRamp.ts` and `spriteMarker.ts` each nudged off the `oklch()` triple `index.css` states. **The shared source walk was attacked directly**: filtering `src/components/quantise/`, then all of `src/components/`, then all of `src/constants/` out of `tests/sourceFiles.ts` fails `design-tokens`, `raw-colour-literals` and `interface-punctuation` every time, so the floors do what their docblocks claim (measured headroom: modules 408 against a floor of 340, components 124 against 100, strings 16,955 against 11,000; the two ground/ink sweeps read 19 and 21 against a floor of 10). **Filed:** #197 (`quantise-column-width.test.ts`'s `SELECT_PANELS` is hand-kept — it omits `AntiAliasControls`' two `max-w-md`-wrapped selects and includes `GridControls`, which holds none, so `max-w-sm` in the first passes while the identical change in `SymmetryControls` fails), #198 (the guidance suite's “never repeats itself” keys on the whole string where its own comment and CLAUDE.md both say sentence — a sentence pasted from `CHROME_TOOLTIPS.history` into `.settings` passes, and a scratch copy of the suite's own discovery reports eleven shared sentences today, including the pair #168 was filed for), #199 (`ControlTooltip`'s docblock says “around fifty” call sites and “seven” disabled-capable ones; the TypeScript compiler API counts 70 across 39 files and 17 across 13, and CLAUDE.md repeats the fifty — the defect #139 fixed for `SelectField` with `select-call-site-counts.test.ts`), #200 (`PRECACHE_SHAPES`' docblock says it describes 15 of the shipped worker's 18 entries; the list holds 45 lines and the built worker 48, 46 unique), #201 (`zipArchive` prices deflate at five bytes per 65,535-byte block; `CompressionStream` emits 16,384-byte blocks, measured at +22 / +65 / +310 raw bytes on 65,535 / 200,000 / 1,000,000 random ones against the docblock's 5 / 20 / 80), #202 (`parseHistoryRow` justifies repairing the two payload columns because “they were added after the first schema shipped” — a pre-1.0-banned compatibility ground for a row `sqliteWorker`'s `PRAGMA table_info` drop makes unreachable; the behaviour is sound as corrupt-storage robustness and there is no dual path, only the comment), #203 (`SheetSplitContents.tsx:52`'s ordinal comment is missing words), #204 (CLAUDE.md bans spelling a whole class name in a comment and the rung scan enforces it for one utility family; a sweep of the 682 selectors in the built stylesheet against the source with comments blanked by `codeOnly.ts` finds eight the app never uses, 789 bytes in all — five deliberate class names in prose and three ordinary English words that collide with a utility, and one of the five is the `bg-ink` tint at 60% that `design-tokens.test.ts`'s own ground/ink docblock spells while banning it, so the banned utility ships and would work). **Commented on #167** rather than filing again: §2's third false claim in the same sentence — “components read through narrow selectors, so editing a subject field re-renders the prompt preview and nothing else” — where `SubjectForm`, `PromptPreview` and `PromptActions` all select `state.subject` whole; a scratch probe on the real store shows `setField` replacing that object and notifying the whole-object subscriber while a one-field subscriber is not notified. **CLAUDE.md re-measured and confirmed:** nine accent hues; ten wheel stops plus the repeated opening stop, so eleven gradient stops; six raw-hex paths, and the three further exemptions the prose names (`differenceRamp.ts`, `spriteMarker.ts`, `aseprite.ts`) carry `oklch()` triples or numeric RGB rather than hex, so the list of six is honest; the type scale at 0.6875 / 0.8125 / 0.9375rem = 11 / 13 / 15px; `TOAST_DURATION_MS` 3000 and `TOAST_EXIT_MS` 2500; the three shell attributes on `App.tsx`; `PRACTICAL_COMPONENT_CEILING` 43; every row of the `test_sprites/` table — eight tracked sheets, the names, the sizes, and all eight 8-bit colour type 2 non-interlaced, read from the IHDR; `src/utils/` genuinely pure (no `document`, `window`, `navigator`, store or `localStorage` reference outside prose and a local variable named `window`); `noInlineConfig`, the four named lint rules, and the `.claude/**` exclusions in both `eslint.config.js` and `vite.config.ts`'s `test.exclude`. **README re-checked** against the tree — Node `>=24`, `.nvmrc` 25, four CI jobs at 25; four tabs; the four settings the dialog carries — and nothing was found; the spec's banner and all five `done/` banners read, and none claims a completion it does not have. **Test doubles cannot drift:** all four worker fakes in `src/test/` are typed against the real protocols (`WorkerCall`/`WorkerReply`, `QuantiseCall`, `AutoTuneReply`, `SheetWriteReply`), so `tsc` catches a protocol change. **Colocated coverage measured:** 246 of 532 non-test modules under `src/`, and `src/utils/` — the directory CLAUDE.md says correctness lives in — is 133 of 155. **Checked and not a defect:** `studio-column-width.test.ts`'s “a container-queried grid in the sticky column” is literally true and Phase 8's hand-off reads it too loosely — the studio's sticky column renders `TargetModelSelector`, `ComponentBudgetNotice` and `PromptPreview`, none of which carries a container query, and `SubjectForm`'s is in the *form* column; `evictionLengths`' “Ten attempts cover a full history” is a bound rather than a figure, nine for both 178 and 200, and its own test asserts at most ten, so Phase 6's hand-off is not a drift; `colors.ts`'s sepia argument is true — all five strings in the app naming Sepia carry `Parchment`, `Cream` or `Vellum Cream` earlier, and `parseColorFromText` takes the first name mentioned; `precache-contract.test.ts` not catching an entry removed from `PRECACHE_SHAPES` is the split its own docblock states, and the build does catch it (`npm run build` in the worktree with the quantise worker's shape removed exits 1 with `PLUGIN_ERROR … precached, not listed`); a ComboBox pool label past 50 characters is correctly outside `select-option-labels.test.ts`, which is scoped to `SelectField` choice lists; `feature-gaps.md`'s “twenty-two `OutputConfig` fields” is a record of what that audit found, which the plan's own ground rules forbid rewriting. **Unproven, for wrap-up:** whether the sentence-level check #198 asks for can be green without an allowlist — nine of the eleven shared sentences are template output or deliberate, and the design work is where the exclusion is expressed rather than whether the check is right; `ControlTooltip`'s third figure, “two of which explain the very condition that disables them”, is a reading rather than a count and was not settled (`GeneratorSiteLink` plainly does, and at least three quantiser download cards end “It is unavailable until a grid is settled”, so the figure looks low); `interface-punctuation.test.ts` calls `ts.createSourceFile`, which never throws and whose `parseDiagnostics` the suite does not read, so a file that does not parse is scanned as recovered tokens and its strings can go unseen — observed when a badly-formed mutation left `chrome.ts` syntactically invalid and the suite passed — but no route was found, because `npm run type-check` fails first on any such file. **Environment, and it bounds this phase:** `C:` had **0 bytes free** throughout, which collapsed a whole-suite `vitest run` with `ENOSPC` across 262 of 277 files. Every run recorded here is per-file with `TEMP`/`TMP` redirected to another volume, and each passed; **no full-suite green was observed in this session**, so the baseline rests on `main` being green when Phase 9 landed rather than on a run made here. **Not driven:** no browser session this phase, and no screen reader. |
| Wrap-up | complete | 2026-08-31 | #205 #206 #207 #208 #209 #210 | Chased all ten phases' Notes: no `partial` rows, thirteen unproven suspicions, six out-of-scope hand-offs and the two questions every phase deferred to Phase 10. **Filed:** #205 (ICON section 8 rescues the selected ring, the highlight halo and the tier mark from its `timer … on a component` ban by name, and not the `Cooldown sweep ×2` section 4 lists — which is the one the ban's own docblock says the word `timer` was written for; both halves present in all five reachable ICON configurations), #206 (`duplicateSprites`' two docblock figures, neither reproducing: built to the docblock's own stated fixture — 512 boxes of 20 × 20, none exact — the pass costs **3,349–3,443 ms** at the top of the dial against "a fraction of a second", 18–20 ms at the bottom against "an order of magnitude less", and "comfortably inside one dial movement" fails; and no reading of "four parts in 255 per channel" reproduces "4 of its 15" — six readings give 3, 6, 1, 2, 2 and 3), #207 (no window- or document-level drag guard exists anywhere in `src/` — grep for `dragover`/`dragenter`/`drop` returns nothing outside `useFileDropTarget`'s two element handlers — so a file dropped off-target navigates away, and `useQuantiseStore` "survives navigation, not a reload"; the navigation itself is still not driven, as Phase 8 found), #208 (`tsconfig.node.json` covers `vite.config.ts`, `scripts/**/*.ts` and all of `tests/` without `noUncheckedIndexedAccess` or `exactOptionalPropertyTypes`, which `tsconfig.app.json` sets; Phase 9's eight diagnostics reproduce exactly, `tests/columnSplit.ts` — shared by both column-budget suites — among them), #209 (the 150-line structural law counts raw lines where its rationale is about responsibility: 155 of 549 non-test modules are over, only **55** on comment-blanked code, and most of those 55 are declarative data — `constants/quantiser.ts` is 2038 raw against 279 code, so ranking by the rule's own measure puts the calibration record top of the list to split; filed `status: needs-design`, since which of three shapes it takes is the maintainer's call), #210 (`Modal`, `HistoryEntry`, `HistoryFooter`, `PresetCard` and `TabSwitcher` are imported by **no test in the repository**, nor are `src/sw.ts` or `scripts/secret-scan.mjs`, and #179, #186, #187, #193 and #194 all went through exactly those files; colocated coverage is 246/549, but `src/utils/` is 133/155 while `components/common/` is 9/26). **Commented rather than filed again:** #169 — both "weaker cases" prove out on the app's own words, so it is five presets and not three (the `anatomy` field's tooltip calls `Shared Backing With Swappable Motif` "a shared plate" while the preset bans "a slot plate … behind the icon", and its card promises "no object behind them"; `Monocular Cyber Eye` against "no facial features" needs no reading at all). **Cross-linked** #186↔#187 (one symptom, two causes, and CLAUDE.md's whole-fix rule), #197↔#199 (both replace a hand-kept list with the compiler-API walk #139 established), #171↔#176 (the sweep drives the scale reading, so fixing one moves the other's figure). **Discarded, with the reason:** INTERFACE's `Icon plate or slot: … filled` against "any gameplay art … inside a frame" — section 8's closing paragraph scopes the exclusions to what a component *shows* and a satisfying reading exists (three plate treatments differing in relief and accent), and no docblock bridges the gap as ICON's does; `describeSeries`' "drawn towards front" against FONT's and EFFECT's "has no front" — **1,620 prompts swept** over category × mode × direction set × rig mode × sheet index and **none carries both**, because the landmark block is gated on `MULTI_DIRECTION`, exactly as those records' own docblocks say; VEHICLE's near/far drive-unit split against one `Drive unit` per yaw — the two never share a section 4 (near/far on the part library and the rig, one-per-yaw on the directional sheets) and the landmark's "a drive unit's front" is singular, agreeing with the directional entry; `frameLattice`'s "It returns 21.33 on that row" — measured, the nine-frame row at 128/6 returns **21.333333** and the five-frame row **21.25**, and the nearest antecedent is the nine-frame sentence immediately before it, so the claim is true as written and the paragraph's argument holds at both; transferring the quantiser's reply instead of cloning it — **safe** (36 configurations over three votes × three grids × reduction × key: the result never aliases the source buffer) but not worth it, measured through `worker_threads` at the app's largest admitted sheet (67.1 MB image + 33.6 MB difference) at **89–101 ms** cloned against 44–53 ms transferred, so ~45 ms against a transform the protocol docblock rightly prices in seconds — its "a few tens of milliseconds" is accurate; `useComboBox`'s unclamped `storedIndex` — the mechanism is real and was reproduced against the real component (highlight walked to 12 of a 16-option pool, pool swapped to 3, `aria-activedescendant` clears and the next ArrowDown lands on **option 1** rather than 0), but the route is closed by a deliberate guard Phase 7 did not name: `handleKeyDown` calls `close()` on Tab, whose comment says "leaving the highlight behind is how it survives to meet a different pool". **Discarded as already captured:** Phase 1's VEHICLE `worn_details` hand-off (#166 lists both options and both presets); Phase 4's median-cut question and Phase 10's `ControlTooltip` third figure (each stated in its own issue's "Not verified" section, as a judgement for whoever restates the paragraph); Phase 10's #198 allowlist design. **Still unproven, and each needs something this machine does not have:** whether Aseprite accepts an RGBA document stating `colors: 0` (no Aseprite build installed); how Midjourney V8.2 reads a multi-word `--no` entry (#152, needs a subscription); whether Alibaba's 800-token note covers the Qwen 3.0 series (#153, needs vendor clarification); what `Run.ps1:340` returns on a Ctrl+C. **Re-searched and still no route found**, so discarded rather than filed: `quantiseWorker`'s unguarded `fail()`; `SqliteBackend.request`'s pending-before-`postMessage` map entry; `useAnchoredSurface` not re-placing a surface that changes size while showing; `useComboBox`'s outside-press listener on the global `document`; the two reduced-motion blocks' differing selector lists; `interface-punctuation.test.ts` not reading `parseDiagnostics`, which `npm run type-check` closes first. **Set reconciled:** every open issue carries exactly one `status:`, at least one `type:` and exactly one `effort:`; both closed ones (#151, #154) carry no `status:`; no issue describes anything a later phase showed to be deliberate. #178 sits inside the range and is the maintainer's own follow-up to #151, not the audit's, so it was left alone. |
