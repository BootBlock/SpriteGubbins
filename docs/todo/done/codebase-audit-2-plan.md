...
> **Status:** ✅ COMPLETE — all thirteen phases and the wrap-up are done. The audit filed
> **forty-three issues**, #220–#222, #228–#248 and #250–#268, and reopened **#98**; five are
> `priority: high` (#239, #241, #243, #246, #254) and six are the effectiveness findings this
> round was for (#243–#248).
...
3. `npx vitest run tests/docs-todo-status.test.ts` green in the worktree. **Not Prettier**: this
   step named `npx prettier --check` on this file for eleven phases, and it passes vacuously —
   `.prettierignore` excludes `*.md`, so `--list-different` on the path exits 0 having listed it
   nowhere. Phase 12 demonstrated that and reserved the correction for the wrap-up, since step 2
   forbids rewriting live guidance mid-audit. A third round copying this checklist should not
   inherit a check that does nothing.
...
| 3B — Adherence scoring | complete | 2026-09-05 | #243, #244, #245, #246, #247, #248 | re-verified: **#178** (its one explicit exclusion is now false — see #245), **#163** and **#235** (read and still applicable; no run in the pack exercises either) | **Scored, on the maintainer’s 27 runs.** The pack at `p:\Source\TypeScript\SpriteGubbins-adherence\` now holds seven scenarios of three runs each — `S1-cardinals` additionally at three reasoning-slider positions, and run D’s three failed attempts — with a `sheet.png` and a `handoff.md` for every one. `scoring.md` is filled: **660 sheet cells (342 satisfied, 147 violated, 171 unmeasurable) and 624 hand-off cells (177 carried, 19 paraphrased, 57 absent, 371 with no transcript)** across 158 figure rows. `_findings.md` beside it states the six confirmed root causes; `_phase3b/` holds the verdict table, the builder that renders the ledger from it and Phase 3A’s original file, so it can be rebuilt. **Six rows were added to the ledger**, each marked `n/a` under Wrapper and Self-audit: Phase 3A extracted the figure list from the compiled prompt’s *figures* and section 1 has none, so the pack’s most repeated sheet failure — the subject coming back as something else, 12 of 27 — had nowhere to be recorded. **Every reading in the notes was re-measured through the app’s own pipeline** — `decodePng`, `keyBackground` at `DEFAULT_KEY_TOLERANCE`, `spriteSegments` at `DEFAULT_SPRITE_GAP`, `buildPalette`, `nearestOklab`, `measureSheetScale` and `estimateProfilePeriod` — and three by-eye readings were corrected, with the correction appended to the notes file rather than written over it. `S1-cardinals/notes.md` and `improving-sol-adherence.md` both scored original run A as satisfying the chirality rule; measured, its west and east views are reflections on all three component rows (flipped IoU 0.93/0.88/0.97 and flipped ΔL 20/18/14, against 0.69/0.76/0.88 and 49/37/29 taken directly), as are all six readable `S1-cardinals` sheets. `S2-nes/notes.md` recorded run A as keeping the visor; read from a 3× crop it has the undercut and no visor, so that scenario’s subject drift is three of three. `S4-native-grid/notes.md` did not name run A’s first row as four **complete buildings**, which section 4 forbids outright. **The palette question `S2-nes/notes.md` left open is answered, and the method is validated.** `buildPalette` recovers the colours the art was drawn in from the resampled sheet and each is priced against the target in scaled OKLab, weighted by the pixels it covers. At a 64-colour budget a positive control — a real sheet forced to 48 colours with `applyLockedPalette`, then put through a box-downscale-and-bilinear-upscale stand-in for the generator’s resample — scores **0.73–0.89** where the 27 real sheets score **2.39–4.99**, with no overlap; against the 55 NES entries the three `S2-nes` sheets score 17.3–22.3 with 2.0–9.0% of pixels within 3 units of an entry, three sheets never asked for that palette score 19.2–24.1 and 2.1–5.1%, and the positive control scores 11.2–13.7 and 14.7–47.2%. **A fourth method was tried and discarded on its control**: distance to the nearest segment joining two palette entries gives the `S2-nes` sheets p50 1.17–1.37 against the NES 55 and a sheet never asked for it **0.99**, better than all three, because 55 colours put 1,485 segments through most of sRGB. **The six issues, each measured:** #243 — every opposite-turn pair is a horizontal reflection, 12 of 12 measurable, with section 3’s rule carried verbatim in 7 of the 7 compositions that state it, and the fix needs the option pools to declare one-sidedness before the per-view enumeration can be derived. #244 — no sheet was drawn inside the 32-to-64-colour budget, 27 of 27, with the figure reaching the image call on 9 of 11 readable runs. #245 — the resolution profile is missed by all 18 character sheets: head plus pelvis alone is 40.1–72.7% of the sheet height against a 25–35% band, and since those two never touch in an assembled figure the bound needs no overlap estimate. #246 — the studio’s own opening configuration returns a male-presenting character 3 of 3 where section 1 states `Feminine`, loses `Fibre-Optic Cabling` entirely and fails the trunk terminations 3 of 3. #247 — `sol.ts`’s docblock records the native-grid block as confirmed fixed on a *composition* test, and three sheets whose directive names it by heading show 41–69 strong transitions on the median across a component that permits 15. #248 — twelve of 27 sheets come back 1536 × 1024 where section 4 asks for a wide 16:9, and every one of them is exactly that size. **The pack’s own largest limitation, stated wherever it bites:** 16 of the 27 interrogations returned a refusal, so **371 of the 624 hand-off cells are empty for want of a transcript** and 16 runs’ worth of sheet losses cannot be attributed to composition or to render. Of the eleven readable transcripts, four open `Edit Image A…` for a render that never happened; on the operator’s direction they are scored at full weight with that caveat on every cell. **Run D is dropped and its column removed** — it asks for the `prompt` argument of `image_gen.text2im`, which the ChatGPT system prompt for this model forbids twice; `sol-image-tool-behaviour.md` carries the argument. **Not measurable, and recorded as such:** figure 6 (render at delivered resolution) and figure 12 (single-pixel outline), because the resample makes a one-pixel contour and a two-pixel one the same thing; figures 8, 21 and 22, which are not on the sheet at all; and **figure 14 (camera elevation), attempted and failed** — looking for the socket ellipse as the darkest enclosed region in the top third of a front-view component finds hair, a visor bar or a panel line as often as a socket, and the implied angles run 3° to 90° with no pattern across 19 sheets. **Out of scope, for the owning phase:** Phase 6 still owns whether `nativeGridScale.ts`’s two gates should make the native-grid block unreachable on the primary scenario; #247 is about what `sol.ts`’s docblock claims, not about those gates. `test_sprites/glyph_sprite_sheet.png` is still untracked and outside `tests/sheetCorpus.ts`’s eight, as the first round’s Phase 3 row already recorded. Eight empty directories are left under `.claude/worktrees/` from other sessions’ failed removals; they hold no `.git` and no files, and were left alone. |
...
| 6 — Quantiser pipeline and its workers | complete | 2026-09-05 | #236, #237, #238 | re-verified: #171 (`vi.doMock` call counter over `measureSheetScale` — one `stepProfile` walk per survey on every corpus sheet, was two or three), #173 (SNAP + drift 0 gives `[10,40,70,100]` under all four `ANTI_ALIAS_MODES`), #174 (`quantiser-docblock-figures.test.ts` re-derives `[4.697, 3.287, 2.447, 1.676]`), #175 (the citation is gone and the table carries a *Grid the run was given* column; every row re-derived by running `autoTune`), #176 (`auto-tune-prologue.test.ts` — mesh, key and hardening once a crop, no difference map), #177 (both comments argue from `mergeIsExempt`; `statedPalette` is called only inside it), #182 (2.34567 → 1.5, 0.71828 → 0, 37 → 100), #183 (`sameSurroundings` withdraws the report; the panel suite green), #192 (the lock takes `resultPalette` and is held shut with a notice on an empty result), #206 (15 boxes, 3 keeping their extent at +4 and 6 at −4). **#98 reopened** — see the Notes. #172 is still open, so per the plan it was neither re-checked nor re-filed; #216 is prompt-compiler and categories rather than this scope, and was re-verified in Phases 1 and 5. The other closed `area: quantise` issues since 2026-08-30 belong to other phases: #179 and #180 to Phase 7, #207, #213 and #214 to Phases 9 and 10, #197 and #209 to Phases 11 and 12, and #219 is the projects feature. | **Everything was audited from the dial, on the eight corpus sheets.** Twenty-six dials were swept through the real `quantiseImage` — each in the context that makes it live, each position digested on the result’s pixels *and* on its boxes, symmetry axes, duplicate groups, strips, palette entries, keyed share and offset — then the auto-tune sweep was run on all eight, the pipeline timed pass by pass on all eight, and the numerical edges probed. Baseline before any scratch file: **307 files, 8,897 tests** green. **Filed (3).** **#236** — five sites say the sprite gap changes no pixel and `QUANTISE_TOOLTIPS.spriteGap` says “the download is the same file whatever it is set to”, but the gap decides the segmentation and three later passes rewrite artwork from it: at `frameAlignment: 'SNAP'` a gap of 8 differs from the opening gap of 1 by **55,065 pixels** on `cyborg_healer.png` (strips 4→3, frames moved 17→12) and 3,700 on `cyborg_monk.png`; under the duplicate fold by **8,249** on `three-quarter-view_tiles1.png`, where the fold also stops happening. `139604b`’s own message lists “the sprite gap changing no pixel” among what it fixed, and the diff corrected only the *uniqueness* half (“the one dial” → “a dial”). **#237** — `SYMMETRY_SWEEP_BUDGET` states 18,073 drawn pixels for the reference sheet’s fifteen sprites; the combined box area `affordableReach` divides by is **17,201** and the opaque count is **13,827**, and none of fifteen readings of the stated conditions gives 18,073. Beside it `symmetryAxis.ts`’s “about a **thirtieth**” runs from **1/12.2 to 1/142.6** across six sheet states and is **1/43.9** at the conditions its neighbour states. Both come from the 2026-08-21 measurement session whose every other figure `a4f74d6` re-measured a day later. **#238** — `DEFAULT_PALETTE_SNAP = 20` is derived from two populations “0.4 apart” on the reference sheet; the pixel-weighted median **0.38** and the top ninety-ninth percentile **20.40** reproduce exactly (which is what establishes the method), while the p90 range, the furthest colour, all four take shares (100.00 / 100.00 / 99.96 / 95.91% against 100 / 99.96 / 99.80 / 93.93%) and the three colour counts (64 / 62 / 59 against 60 / 63 / 59) do not — and the six absent colours the upper population is defined by are named in words with no hex, so that half cannot be re-derived at all. **Reopened (1).** **#98**, on CLAUDE.md’s first reopening ground: the docblock records a settled reading the sweep does not produce, now with the two readings swapped. Both passages say `DOMINANT`; the sweep settles `K_CENTROID` on `armour.png` (0.6554/112.2), on `cyborg_healer.png` (0.6539/196.2) and on all eight. The stage figures they quote all still reproduce — what moved is #134’s rounds, which re-choose the reading in round two. `type: regression` and `status: ready` added; effort raised small → medium. **Re-verified, nothing to file.** Every figure in `constants/autoTune.ts`: the 145 / 1161 / 1217 / 857 / 889 arithmetic against the ladders (15, 49, 11, 15, 13, 4 | 10, 8, 20); the eight-row `TUNE_ROUNDS` table to the digit (6/403, 4/181, 4/185, 3/142, 3/142, 3/142, 3/134, 3/134); 403 positions over six rounds ending 0.6554/112.2 from 0.6264/960.4; the `BOTH` variant at 301/4, floor 40, strength 30%, 0.6568/301.6 from 0.6399/1058.2; the budget-16 `BOTH` eight-round claim; the 240 px crop, its twenty-seventh and the just-under-a-fifth; five crops on all eight. The whole symmetry ladder: 15 sprites at 23–34 px across with the quarter-width bound binding on 10 of 15, 11,912 colours, 0.7/8.0/14.8/24.2/35.6/52.7/77.1%, the reduced sheet at 11 colours flat 36.9% to 24 and first moving at 25, shares 18–68%, none settled at 90 or 75, one at 65, two at 60, four at 55. `COLOR_MERGE_RANGE`: 64 → 27 at 12, 14 at 24, 6 at 48. The auto-tune report matches the dials it reports on all eight sheets, re-read through `readCandidate` on the sweep’s own prologues. **Numerical edges, all handled:** a 400×1 strip, a 1×400 column, a 1×1 sheet, a grid past the sheet, a fully keyed sheet, a sheet whose key colour is also a sprite colour and an 8192×512 sheet all pass through `quantiseImage` and `measureSheetScale`; `autoTune` refuses a sheet smaller than one cell in as many words. **Worker lifetime clean:** `useSheetWriteStore.writing` is cleared only by `sheetWriteSession`’s own `finish`, which terminates in the same breath; `abandonSweep` sits beside `releaseSheet` in both `setSource` and `clear`; every reply path checks `jobs` or `owns(run)` before filing. **Judged not findings.** `mergeColors` and `despeckle` each allocate a full copy before their off-position early return, against the rule `quantiseImage.ts:205-211` states for the outline expansion — measured at 1.3ms of a 1,476.9ms transform (0.1%) and 8.5 + 8.2ms at the 4096² ceiling, too small to file. The quantiser reply is cloned rather than transferred; Node’s `structuredClone` at that ceiling costs 17.8ms for the sheet and 36.6ms for a reply carrying the difference map against 8.2ms transferred, so `quantiseProtocol.ts`’s “a few tens of milliseconds” holds. `applyPalette` is 32–48% of the transform on every corpus sheet — the dominating stage — because `DOMINANT` reduces before the vote, which that file argues deliberately. `silhouetteThreshold` is inert on all eight because they are colour type 2 and `keyBackground` writes only 0 or 255 alpha, which `KeyingControls.tsx:181-190` says outright. The 99-versus-100 cell count on a short axis is `boundEndCells` merging a band under `shortestEndCell`, documented with the reference sheet’s own 210×209. **Out of scope, for the owning phase.** Phase 10 — `SpriteControls.tsx:50-53` carries #236’s claim, so the component half of that fix lands there. Phase 12 — `tests/quantiser-docblock-figures.test.ts` reaches `constants/autoTune.ts` not at all, and the symmetry and palette-lock ladders only through the sprite count of 15; three of this phase’s four findings are figures that suite would have caught. Running notes at `<scratchpad>/audit2-p6-notes.md`. |
| 7 — Downloads, encoders and atlas maths | complete | 2026-09-05 | #239, #240 | re-verified: **#179** (120 separated blocks through the real `writeSheet`; the archive’s central directory reads `sprites/001.png` … `sprites/120.png` and its own order equals its sorted order, `spriteOrdinal(index, count)` deriving the width from `String(count).length`), **#180** (two facings of one batch share no entry name at all — `south-sheet.png` / `south/01.png` / `south-manifest.json` against `north-*` — and neither do two *facingless* sheets of one batch, `sheet-1-*` against `sheet-2-*`, which is the `sheetToken` half added after that issue closed). None reopened. **#201 is still open**, so per the plan it was neither re-checked nor re-filed. The only closed `area: sheet-splitter` issues since 2026-08-30 are #203 and #162, both re-verified in Phases 1 and 5; `area: atlas-calculator` has none. | **Everything was audited from the file a reader gets, read back by software sharing no code with the writer** — Python 3.10’s `zipfile`, `zlib` and `struct`, a from-scratch PNG decoder that reconstructs pixels through all five filters, and an `.aseprite` parser written from the published format specification. Sheets came from the app driven in Edge on `test_sprites/armour.png` at grid 6 with the studio’s magenta key, at 1× and 4×, bounding-box and fixed-cell cuts, plus synthetic edges through the real `writeSheet`. **Filed (2).** **#239** — the pack and the manifest order sprites by **first scanline** rather than by section 4’s “screen-left to screen-right, then top to bottom”, because `mergeNearbyBoxes.ts:64` sorts lexicographically on the exact `top`. A row whose sprites differ by a pixel is interleaved: on the driven `armour.png` download, five of the fifteen cut-out PNGs carry another component’s name — `sheet-1/01-heads-front.png` holds the head that is *second* across, and the first across arrives as `04-heads-back-three-quarter.png`. Seven of the eight corpus sheets are misordered (5/15, 6/15, 4/15, 22/43, 18/31, 21/25, 11/27; the tile sheet alone is clean, its rows being flush by construction), and on every one of those seven the `.aseprite` frame order — which `sheetLayout` re-derives correctly — disagrees with the manifest, so one press-set writes two files that number one sheet two ways. It reaches `ManifestSprite.index`, which the type’s own docblock offers as the join key to the prompt’s component map, and the oversize refusal, which names the piece by the same index. **#240** — `useFileSave` says “both of the app’s downloads” and “the two callers” where there are three, and `useDownload` still calls the JSON pack “the preset pack”; the palette download landed 2026-08-23 in `3117d65`, a day after `useFileSave.ts` was last edited, and the library consolidation merged the two packs since. **Checked and clean.** Every PNG the app writes passes a strict decode — signature, per-chunk CRC recomputed, chunk order, IHDR against the inflated IDAT length, filter bytes in range, every index inside `PLTE`, `tRNS` a strict prefix — across indexed and truecolour, 1 × 1, all-transparent, semi-transparent-only, mixed-alpha and 4096². Every zip’s central directory is compared field by field with the local header it points at, its CRCs recomputed over the stored bytes, and `zipfile.testzip()` answers `None`. The `.aseprite` files parse chunk by chunk against the specification with every frame’s own size field, every reserved run, every `STRING` length and every cel’s inflated size checked, and the background-layer decision behaves exactly as `aseLayer.ts` documents. Every sprite PNG in a pack matches the sheet pixel for pixel at the rect its manifest states, at both magnifications and both cuts. The `.gpl` satisfies GIMP’s grammar, in ASCII with LF; the swatch is 992 × 16 for 62 entries. 58,344 component names across every category × mode × direction set × sheet index are filesystem-safe path segments, and no two downloads of one session collide. The atlas maths swept over 20,100 configurations never overflows the texture, never reports an impossible share, and prices every texture exactly as an independent block count does (chain/base 1.3333–1.3334, matching the tooltip’s “roughly a third again”); `smallestCanvasFor` is correct and monotonic over 420 cases; `widthBiasFor`’s docblock figures reproduce exactly (2.3379, 11 × 4 at 32%, 7 × 7 at 83%), as do `encodePng`’s own (13,649 bytes unkeyed, 10,312 keyed, to the byte). **Judged not findings.** `describeWriting` gives a truecolour SPRITE_PACK Aseprite’s “written in RGB colour mode” where the PNG branch says “truecolour” — one clause, on a state reached only with no colour budget in force, and the docblock shows the wording was deliberated. `crc32` iterates with `for…of` and is 2.5× slower for it (4.2ms against 1.7ms over a 193,423-byte IDAT), which is nothing beside the 3,171ms a whole truecolour `encodePng` costs at `MAX_IMAGE_PIXELS` — 2,700ms of it the five-candidate filter trial, which that file argues with its own measurement. The header’s `colors: 0` on an RGBA sprite is a specification gloss and unverifiable without Aseprite, which is not installed. **Out of scope, for the owning phase.** Phase 1 — `sheetLayout.ts`’s docblock bullet “The canvas is the largest sprite” is contradicted by the inline comment four lines below it and by the code, which takes the *deepest band*: measured, `cyborg_healer.png`’s canvas is 2px deeper than its tallest sprite. Phase 6 — #239’s root cause is the sort in `mergeNearbyBoxes.ts`, which is that phase’s scope; it is filed here because the defect is in what the download writes, and the issue names the level the fix lives at. Phase 10 — switching the Sprite cut control to Fixed leaves the cell at 1 × 1, so every sprite is oversized until a size is typed. Phase 12 — the plan names `src/utils/quantisePresetPack.ts` in this scope and no such file exists (the two pack collections are now `libraryPack.ts`), and it asks for “the File System Access path against the anchor-download fallback (both exist)” when only the anchor path does — `showSaveFilePicker` appears nowhere in `src/`, so a cancelled picker is not a reachable state. Running notes at `<scratchpad>/audit2-p7-notes.md`. |
| 8 — Stores, persistence, session and history | complete | 2026-09-05 | #241, #242 | re-verified: **#181** (the fallback's `LocalStorageBackend` over `createMemoryStorage` lists `['Charlie','Bravo','Alpha']` after three saves and `['Alpha renamed','Charlie','Bravo']` after a rename; the shipped `CREATE_TABLES_SQL` / `INSERT_PRESET_SQL` / `SELECT_PRESETS_SQL` through `@sqlite.org/sqlite-wasm` give the same two lists — `savePreset` now goes through the shared `upsert`, which prepends, and `toPresetRow` / `parsePresetRow` are one shape on both sides), **#182** (`parseQuantiseDials` turns 2.34567 into 1.5 and 0.71828 into 0; `paletteSnap` 37 stays 37, which is in range and on its step of 1 — Phase 6's log records that one as “37 → 100”, which is not what this dial does, and the defect itself is gone), **#183** on its store side (`project()` in `useQuantiseStore` retires the report on every dial path but the sweep's own, and `stale()` is the only thing that clears it). #209 and #219 are the other closed `area: persistence` issues since 2026-08-30 and belong to the module-size law and the Projects feature. **#202 is still open**, so per the plan it was neither re-checked nor re-filed. None reopened. | **Everything was audited from a session's lifetime, driven in both persistence modes and diffed.** The same sequence — edit the studio, copy a prompt, save a preset into Default, make a second project, save a second preset into it, copy again, load `test_sprites/armour.png`, set a pixel grid, save a quantiser preset, export the library, reload, re-import — ran on SQLite/OPFS and on the localStorage fallback (forced by aborting every `.wasm` request, so `sqlite3InitModule` rejects inside the worker and the handshake answers `ready: false`). **The two end states are identical** at all three snapshots, and both packs are 208,255 bytes with the same 2 projects, 2 custom presets among 101 entries, 1 dial set, and the same “Imported 5 saved items, replacing 5”. **Filed (2).** **#241** — a second tab of the same origin cannot take the SAH pool (`NoModificationAllowedError: … createSyncAccessHandle …`, logged seven times), so `openSqliteBackend` answers `null` and the tab silently runs on localStorage: its Projects panel opens on `Default 0` over a library that is not empty, `fetchProjects` then writes a *second* Default project into that store, and “FromTabTwo” saved there is unreachable the moment the first tab closes and the second reloads onto SQLite. The handshake is one boolean and its docblock argues that no reason could be acted on — which is true of the three causes it names and false of this fourth one. No decision record covers a second tab. **#242** — `recordDials` coalesces an edit with the entry an **Undo** has just landed on, because `extendsCurrent` compares against a timestamp written before the undo: record, record, undo, record at t=600 keeps 2 entries where 3 are right, and driven in Edge the Dial history badge reads `1 step back` where `2 steps back` is right, at 0/100/200 ms between acts and correctly at 300/500/800 ms. **Sweeps run, all negative.** Every storage parser fuzzed field by field — `parseImageConfig`, `parseOutputConfig`, `parseSubject` (13 categories × 16 fields), `parseSettings`, `parseSettingsRow`, `parseSession`, `parseSessionRow`, `parseQuantiseDials`, the four row parsers, the three imported-row parsers and ten hostile pack texts, each slot given 22 adversarial values: **0 problems**, with every union member checked against its own `as const` array, every number finite and in range, and every pack preset naming a project the pack holds. The two backends driven through one scripted session (3 projects, 3 presets, 2 dial sets, 6 history rows, a delete, two upserts, a project cascade, settings, a session row and a `replaceLibrary`) answer identically on all six collections. Twelve store actions run against storage that refuses every write leave the store and storage in agreement in eleven cases; the twelfth is `updateSettings`, whose divergence is the documented “Applied, but that setting could not be saved”. The studio stack across a category change, a preset load and a history restore runs 0→1→1→2→3, the un-recorded field edit made after the switch survives the undo and returns on redo, and a library import leaves the stack byte-identical. History eviction with real compiled prompts (27,317 characters each) keeps **136** entries at 3,998,673 characters against the 4,000,000 budget with no refusals, on an unbounded store and on a 5,200,000-character one alike; a prompt larger than the budget is rejected and leaves the stored history untouched. `ORDER BY updated_at DESC` with every key tied returns insertion order at 200, 1,000, 5,000 and 20,000 rows, which is the order the fallback stores. The library pack round trip is idempotent from the first parse onward. No `use*Store(` call site in `src/` subscribes wholesale. **Confirmed as documented, not filed.** A schema change was driven for real: `custom_presets` given an extra column in the DDL and in `TABLE_COLUMNS`, the same browser profile reloaded — `Default 1` became `Default 0`, the saved preset gone, projects, history and the session untouched. That is the discard `TABLE_COLUMNS` describes; the reader is told nothing, which is the pre-1.0 bargain rather than a defect. The source was reverted and `git diff` is empty. Session lifetime: an edit reloaded 120 ms later survives, and so does one whose tab is closed 120 ms later, against a 400 ms debounce — the `visibilitychange` / `pagehide` flush works. The studio and the loaded sheet survive in-app navigation; the sheet is discarded by a reload, as `useQuantiseStore` says. **Unproven, recorded not filed.** Two saves inside one millisecond do diverge between the backends — SQLite returns `p1,p3` where the fallback returns `p3,p1` — but a reader cannot make two presses land in one millisecond, and every reachable path was consistent. The session-restore race could not be produced either: on a cold start with a warm database the restore had already landed at the first sample after the field became visible (611 ms), so there is no window in which a reader's typing is overwritten on this machine. **Out of scope, for the owning phase.** Phase 9 or 10 — `src/components/projects/` draws only the selected project's contents, and which project is selected is that view's own state. Phase 12 — nothing under `tests/` or in a colocated suite covers a second tab of the same origin, and `src/utils/dialHistory.test.ts` has no case where an edit follows an Undo inside the coalescing window, which is what would have caught #242. Running notes at `<scratchpad>/audit2-p8-notes.md`. |
| 9 — UI primitives, chrome, modals, tabs | complete | 2026-09-05 | #250, #251, #252, #253 | re-verified: **#185** (tapped a wrapped control in a touch context — no card at 0/600/2100 ms, where a mouse hover on the same control shows one at 900 ms), **#188** (`toast-timer`’s **progress** 0.668 → 0.724 → 0.933 → 1.000 across the boundary and 0.492 → 0.579 the other way, with no `toast-in` running and `opacity: 1` throughout — reading `elapsed` reproduces the original 2008 → 150 and is the quantity the closing comment says will necessarily restart), **#190** and **#198** (the whole suite green, which holds `design-tokens.test.ts`’s sweep of every `--color-tab` assignment in `src/` and the guidance sentence check), **#204** (`npm run build` green, which runs `deadUtilityPlugin` from `closeBundle`), **#207** (a `DragEvent` carrying a real file `DataTransfer` dispatched at six control kinds — `preventDefault()` and `dropEffect: none` on all six), **#212** (the card is 510 characters and the account under the control differs for all three options), **#213** (the same dispatch with `text/uri-list` — refused everywhere but a text and a number input, which is the specified behaviour), **#214** (a file `dragover` on `#main-content` is claimed and the veil appears). None reopened. **#184, #186, #187, #189 and #195 are open**, so per the plan they were neither re-checked nor re-filed — #184, #186 and #187 were met anyway while driving and each description is still accurate (tap 1 shows the ⓘ’s card, tap 2 closes it, taps 3 and 4 do nothing; focus lands on `<body>` after a two-press confirmation and after Escape on all four overlays). | **Everything was audited from the keyboard and the accessibility tree, in a driven Edge 152.** The five views and the four overlays were walked with real Tab presses, each stop’s accessible name taken from Chromium’s own tree rather than from attributes; then contrast, forced colours, `prefers-contrast`, both reduced-motion routes, class emission, listener accounting and the detached window. Baseline before any scratch file: **307 files, 8,897 tests** green, and `npm run build` green. **Filed (4).** **#250** — Escape dismissing a guidance card inside a `<dialog>` closes the dialog too, on both the keyboard and the pointer route, and `useTooltipReveal`’s docblock closes the question off with a measurement that no longer holds: of the three remedies it names, `preventDefault()` on the `keydown` **does** keep the dialog open (`open=true`, no `cancel` event) and so does `preventDefault()` on `cancel`, while only `stopImmediatePropagation()` fails. 37 guidance triggers are rendered inside a `<dialog>` before the per-row ones are multiplied. **#251** — the app’s own `@media (forced-colors: active)` rule paints `HighlightText` on `Highlight` for `[aria-pressed='true']`, `[aria-current]`, `[data-active='true']`, and Chromium’s `Canvas` text backplate then hides the text: the active view’s label measures **8 distinct colours in the light forced palette and 4 in the dark one** where the same 38 × 18 box holds 98 with forced colours off, 91% of it flat `Canvas`; the preset library’s current collection gives 3 against 140. `Canvas` and `HighlightText` are the same colour in both of Chromium’s palettes. Two tests pin the rule that causes it. **#252** — three repeating lists give every row’s action one name: 23 × “Load preset”, 3 × each of “Copy prompt” / “Restore” / “Delete this prompt” per history entry, 6 × “Copy this sheet”, against the projects view’s own “Load the preset Bravo into the studio” for the same action. **#253** — every placeholder in the app is Tailwind’s preflight `color-mix(in oklab, currentcolor 50%, transparent)`, which no token names and neither ground/ink sweep can see: **4.443:1 measured from decoded screenshot pixels** and 4.48–4.49:1 computed on all **24** placeholder-bearing inputs. **Sweeps run, all negative.** Rendered contrast over **508 text elements** across five views and three overlays — zero under threshold, the one flag being the chrome’s Copy Prompt, whose gradient ground this method cannot price and which correctly wears `text-foundry-950`. Class emission both ways: 574 emitted selectors against 494 class tokens spelled in the app’s markup, and the two apparent misses are tokeniser artefacts (`after:content-[` split inside an arbitrary value, and `TONE_CLASSES[tone]`) — nothing the app wears fails to emit. No `duration-[…]` and no `animate-[…]` anywhere in `src/`; the only inline `animationDuration`/`animationDelay` are `ToastCard`’s, from `TOAST_DURATION_MS` and `TOAST_EXIT_MS`. Both reduced-motion routes collapse all 16 running animations to `1e-05s` and leave no transition with a non-zero duration. `prefers-contrast: more` changes nothing, and nothing in the app claims it does. Listener accounting under React 19 Strict Mode is flat across 3 × 5 navigations, 3 × 3 overlay open/close cycles, 3 × 8 guidance reveals and 3 × 6 combo opens — `document` `mousedown` stays at 16, the DOM at 794 nodes. The detached preview window closes on navigation away and on reload, returns to the page when closed from its own side, reports `crossOriginIsolated === true`, mirrors `lang="en-GB"` and carries both canvases. The combo box’s grammar is right at every stage: focus opens, ArrowDown from nothing takes option 0, ArrowUp wraps to the last, Escape closes and clears `aria-activedescendant`, a second Escape is inert, typing reopens with the highlight cleared, Enter commits — and `aria-activedescendant` never names an id absent from the DOM. The skip link moves from `top: -96px` to `top: 16px` on focus, Enter puts focus on `#main-content`, and the next Tab lands on the view’s first control. No stop in any of the four overlays lacks an accessible name. **Judged not findings.** The accent swatches go blank under a forced palette, which `SettingsAccentField`’s docblock reasons about explicitly — only the ✓ is affected, and that is #251. `aria-autocomplete="list"` on the deliberately unfiltered `ComboBox` is right: ARIA’s `list` says a collection of values that *could* complete the input is displayed, and `none` would claim no list is shown. `<summary>` reported as `heading` is a Playwright `ariaSnapshot` mapping artefact, not a missing role. The `Reduce motion` checkbox reading `[disabled]` under a system preference is `aria-disabled`, in the tab order, with the reason as its description, exactly as the component documents. **Out of scope, for the owning phase.** Phase 10 — `SheetProgress.tsx:66` is an `aria-live="polite"` region whose text changes to “Copied” on the same press that raises the toast, so one copy is announced from two regions; and `PromptPreview`’s `<pre>` is a tab stop with no accessible name and no `role="region"`. Phase 11 — the forced-colours block and the `forced-color-adjust` guard are `index.css`-as-a-stylesheet, which that phase owns; #251 names both. Phase 12 — nothing under `tests/` checks that two controls in one list carry distinct accessible names, and nothing prices a pseudo-element’s contrast, which is the gap #195 and #253 share. Running notes at `<scratchpad>/audit2-p9-notes.md`. |
| 10 — Studio and quantise views | complete | 2026-09-05 | #254, #255, #256 | re-verified: **#185** (a touch on the studio panel's Copy Prompt opens no popover at 0/700/2300 ms, where a mouse hover on the same control opens one), **#192** (the fix took the *withhold-the-press* route rather than the toast route — a 96 × 96 all-`#FF00FF` sheet at grid 4 with keying on gives `Quantised · 24 × 24 · 0 colours`, the button `disabled === true`, and the panel names the reason), **#197** (`SELECT_FILES` is now discovered through `reachableFrom(TAB_GRAPH)`; mutating `max-w-md`→`max-w-sm` in `AntiAliasControls` now *fails* the suite — `expected 384 to be greater than or equal to 442` — where the issue recorded it MISSED), **#207** (a real-file `DragEvent('dragover')` dispatched at seven studio targets, including the prompt `<pre>` and `#main-content` — `preventDefault()` and `dropEffect: 'none'` on all seven), **#212** (the card is 510 characters and the chosen option's account renders under the control), **#213** (the same dispatch with `text/uri-list` — refused everywhere but a text and a number input), **#214** (a file `dragover` on `#main-content` is claimed and the veil appears), **#183** (swept `armour.png` at grid 3 to `Swept 142 positions and moved the dials. Likeness 0.689 at 19 colours, from 0.684 at 64.`, then changed the pixel grid to 4 — the report is withdrawn and the panel falls back to its guidance), **#219** (the Projects control and the preset `Save as` box are present in both views). None reopened. **#191 is open**, so per the plan it was neither re-checked nor re-filed, and nothing here makes its description wrong. | **Filed (3).** #254 — `identityPalette` drops the background key by matching `#FF00FF` **exactly**, and **not one pixel of any of the eight corpus sheets is exactly `#FF00FF`**: the generators resample, so the key arrives as a spread covering 52.7–73.7% of the sheet and the coverage ordering puts it *first*. All eight digests lead with a magenta (`#F503F8`, `#F603FA`, `#EE04EE`, `#FB03FB`, `#FA04E6`, `#F802F1`, `#F803F9`, `#F703FA`), and carried through `withPaletteSegment` → `generatePrompt` at the studio's opening configuration the compiled §5 block reads `Palette: #F503F8, #185B23, #000000, #AB8D4E, #BDA15E, #FFFFFF` under “Reproduce exactly” and “the identity lock wins”. Ground rule 1's exception: the docblock's rationale is an argument about a *uniform* key leaving a one-pixel fringe, and CLAUDE.md's own corpus description already records the measured spread that falsifies the premise. The control's copy states both false facts in as many words — “with the background key excluded”, and blends “into the tail of the list”. Confined to the picker and drop routes; `QuantisedSheetCaptureButton` is guarded by `borderKeyShare` and by `subjectPixels`' alpha branch. #255 — the compiled prompt's `<pre>` is a keyboard tab stop with no role and no accessible name and 15,025px of scroll, and a sweep of *every* overflowing element in both views shows it is the only one: the quantiser's two panes carry `role="group"`, an `aria-label` and `tabIndex={0}`, which is the rule `PanViewport` writes down and this call site does not use. #256 — one Copy Prompt press changes **two** `polite`/`atomic` regions with near-identical text (`Sheet 1 of 6Directional core · 5 facingsCopied` and `Copied sheet 1 of 6 — Directional core · 5 facings`), by both routes into the action; `SheetIdentityControls`' identical region on the other tab is unaffected because it carries no copied badge, which is the shape the studio's strip is one badge away from. **Checked and clean.** Every preview-mode switch, zoom step and difference-scale step on `armour.png` at grid 3 costs 4–14 ms pointerdown→commit with **zero** long tasks, so there is no main-thread canvas work to move. 19 studio selects at 1120/1121/1224/1225/1400px and 6 quantise selects at 1224/1225/1400px are all wider than their longest option's budget. The linked panes hold their place exactly: pane 0 to (300,200) moves pane 1 to (300,200), 2× gives both 764/591 and 1× returns both to 300/200, wipe re-centres both to 127/200 as the pane widens 327→673, and the detached window opens with both at 585/404. Edge images all reported through the toast — a `.txt`, a 1 × 1, 4200 × 4200 (`past this app's limit of 16,777,216`) and 12000 × 200. **No wholesale store subscription and no fresh-literal selector anywhere in scope**; eleven components select `state.output` by reference, but each reads several output fields and the children re-render because their parent did, at 22–26 ms per keystroke including a full 28,000-character compile, one commit, no long tasks. **The instrument, because the naive version is wrong:** render counts come from a DevTools hook stub that reads `fiber.flags & PerformedWork` and then **clears the bit on the fiber and its alternate** — without the clear a bailed-out subtree keeps last commit's flag, and a bare Tab press reported 29 components instead of 2. **Judged not findings:** `PromptBudgetNotice` prints `used`, `limit` and the excess without thousands separators while `useImageFile` writes `16,777,216` — Phases 2 and 4 both weighed this and the notice was re-verified as firing correctly and only at `SEEDREAM`, `QWEN_IMAGE`, `STABLE_DIFFUSION` and `FLUX`; and two exported functions are both named `sheetIdentity` (`sheetRuns.ts` returns a string, `sheetIdentity.ts` an object), a naming hazard with no demonstrated defect. **Handoffs dropped with the reason:** Phase 7's sprite-cut-at-1 × 1 is a documented decision whose rationale holds — the badge says `N sprites larger than 1 × 1` immediately and the download refuses; Phase 6's `SpriteControls.tsx:50-53` is already inside open #236's own `## Where`. **Harness trap worth knowing:** the Bash tool's *quoted* heredoc still collapses `\\` to `\` on this machine, so `\\s` written into a JS template literal reaches the file as `\s` and the literal reduces it to `s` — which silently turned a probe's `.replace(/\s+/g,' ')` into `.replace(/s+/g,' ')` and deleted every lowercase `s` from its output. Probe files were written with the Write tool afterwards, and no issue body quotes a string produced through that path. **Out of scope, for the owning phase:** Phase 11 — nothing under `tests/` checks that a keyboard-scrollable region carries an accessible name, which is the gap #255 went through and the shape #252 records for duplicate names. Phase 12 — the column-width suite now discovers its own panels, but nothing measures a *rendered* select against its longest option, so the derivation and the measurement remain two separate claims. Running notes at `<scratchpad>/audit2-p10-notes.md`. |
| 11 — Shell, PWA, tooling, CI, types | complete | 2026-09-05 | #257, #258, #259, #260 | re-verified: **#193** (driven end to end — two production builds on a Pages-shaped host sending no COOP/COEP and `Cache-Control: max-age=600`; v1 installs 58 entries, v2 deploys, `controllerchange` at 0.5s, the old cache is deleted and the new one’s precached shell names the **v2** entry chunk, 11 files re-fetched — the 7 revisioned plus the 4 whose hash moved — and a returning visitor with the HTTP cache emptied by CDP rendered with **zero** requests reaching the host), **#194** (the real scanner in `--staged` mode over a 7-line probe: all 7 reported, where the issue’s own run reported 3 of 7), **#196** (20 `##` sections against 20 table rows, `tests/agents-rule-table.test.ts` green, and AGENTS.md now says “four documented exemptions” and “one of the two `@theme` blocks”), **#204** (`npm run build` green, which is where `deadUtilityPlugin` runs), **#208** (`tsconfig.node.json` carries `tsconfig.app.json`’s strictness block verbatim, `tests/tsconfig-strictness.test.ts` compares them, and CLAUDE.md’s table now names both configs), **#209** (the target counts code, `tests/module-size.test.ts` green; re-derived today — 589 non-test modules, 169 raw > 150, **47** code > 150, every one inside `DECLARATION_PATHS`), **#211** (UTF-16LE, UTF-16BE, UTF-32LE and UTF-32BE probes, each `- -` in `--numstat`, each reported; `STRIDES` is `[1,0] [2,0] [2,1] [4,0] [4,1] [4,2] [4,3]`, exactly what `tests.yml` claims). None reopened. #197, #198 and #206 fall in scope only through the `area:` sweep and were re-verified by Phases 10, 9 and 6. | **The PWA, isolation, CI and tooling half is clean, and it was driven rather than read.** Offline: all five views render with no failed request, and a deep offline session — expand-all, Copy Prompt, the history drawer reading a logged prompt back out of SQLite, the atlas calculator, settings, and the quantiser on `test_sprites/armour.png` at grid 3 giving `Quantised · 422 × 417 · 64 colours` — reached the host **twice**, both times for `sw.js`. First visit: not isolated at first paint, `coi-bootstrap.js` reloads once on `controllerchange`, then `crossOriginIsolated === true` with `SharedArrayBuffer` present. The **same-origin gate** was re-measured against a second origin sending no CORP and reproduces all three shapes CLAUDE.md records — a plain script element blocked, a `crossorigin` one loaded, a same-origin URL that 302s off-origin blocked — with the app’s own responses carrying all three headers and the redirected one carrying none, which is `response.url` deciding as the docblock claims. **CLAUDE.md’s tool-exclusion table re-probed** from the primary checkout with a worktree `dist/` present and a deliberately malformed `probe-leak.test.ts` inside the tree: `git status`, `prettier --list-different .`, `eslint .` and `tsc -b --noEmit` clean, and `vitest run` collected 309 files / 8962 tests — identical to the worktree’s own run. **Secret scan**: whole tree exits 0, and the whole history (`git log --all -p` through the real `scanAddedLines`) flags 0 lines. **Filed (4).** #257 — the three type-aware async rules are enabled for `src/**/*.{ts,tsx}` alone, so a floating promise and a `forEach(async …)` in `tests/precache-contract.test.ts` pass `npm run lint` with 0 errors while the identical lines in `src/utils/isolationHeaders.ts` are 2; this is #208 one layer up, and CLAUDE.md’s banned-patterns row names both rules unscoped. #258 — `VOTE_METHODS` and `DITHER_PATTERNS` in `src/types/quantiser.ts` say the value is never persisted and nothing validates it, while `parseQuantiseDials` checks both (`:75`, `:113`) and a stored `NOT_A_VOTE_METHOD` comes back `DOMINANT`; four further docblocks cite the false pair as their contrast case. Made false by `5d0c6cd`. #259 — `.gitignore:60-61` says the pre-commit scan “cannot flag a binary .sqlite”, which `d7aba3f` (#211’s fix) falsified and did not update; it is the only site of that claim. #260 — `ManifestCell` and `SpriteCell` are the same shape declared twice with nothing relating them: a field added to `SpriteCell` produces 13 errors, none in the manifest’s types or at the assignment that hands one into the other, so the declared contract for an emitted file can stop describing it. **Commented, not filed.** #200 is open and its reported site is fixed — `76c7fd1` replaced “15 of the shipped worker’s 18 entries” with a relationship, the durable route the issue asked for — but the same restated-count defect now sits in `src/sw.ts:80-81` (“39 of the 46 in this build”) and `src/sw.test.ts:12` (“thirty-nine hashed assets”), where the figure is **51 of the 58 unique** today; per the plan an open issue in scope is commented on rather than re-filed. **Candidates closed, not findings:** the precache contract’s blindness to `additionalManifestEntries` is documented at `precacheContract.ts:52-57` and the appended entries carry `size: 0`, so the ceiling is not inflated (verified in `workbox-build/build/lib/transform-manifest.js:38-44`); the forced-colours `.border-neon` rule reaches only `PanViewport`, because `Badge` and `PromptPreview` wear `border-neon/40` and `/30`, which are different class tokens; `BeforeInstallPromptChoice` is the platform’s own event shape and is reached through `installPrompt.userChoice`; `isCustom?: undefined` is never narrowed at runtime but does type-level work; the `inlineDynamicImports` deprecation warning comes from `vite-plugin-pwa`’s own build, not from this config; Prettier’s effective set matches the pre-commit hook’s grep exactly, because `.prettierignore` excludes `*.md`, `*.yaml`, `*.yml` and `*.html`; all 63 `@theme` tokens are reached and the two reduced-motion blocks carry identical declarations selector for selector; every label `dependabot.yml` and both issue forms name exists (58, the figure `config.yml` states) and both `contact_links` resolve 200; and `repro.log` is absent and `*.log` git-ignored, which is how “decide `repro.log`” resolves. **Workflows read job by job**: no gate a dispatch can skip (`release` → `gate` → `build` → `deploy` → `tag`, each needing the last), every job checks out the caller’s resolved sha, and the claims that could be checked were — the byte-scan spacings, the four required artefacts, the least-privilege grants, and `npm version 0.13.1 --no-git-tag-version` touching only the two version fields, which is what `deploy.yml:88-92` promises. **Not a finding, recorded:** `npm install` on npm 11.6.2 / node 25.2.1 rewrites `package-lock.json` (18 insertions / 33 deletions of `peer` metadata and two bundled `@emnapi` entries, reproducible twice, entering at Dependabot’s `07078e7`), which dirties every worktree against CLAUDE.md’s `git add -A` step; CI runs `npm ci` and is unaffected, so it is judged toolchain drift rather than a repository defect. `npm audit` reports one high advisory — `nanoid@3.3.17` through `vite` → `postcss`, build-time only — and an open Dependabot branch already carries the vite bump. **Out of scope, for the owning phase:** Phase 12 — Phase 10 handed “nothing under `tests/` checks that a keyboard-scrollable region carries an accessible name” to this phase, but `tests/` is Phase 12’s; whether to close or re-scope #200; and `npx prettier --check docs/todo/codebase-audit-2-plan.md`, which the completion checklist names, is vacuous because `*.md` is in `.prettierignore`. Running notes at `<scratchpad>/audit2-p11-notes.md`. |
| 12 — Tests, documentation and the backlog | complete | 2026-09-05 | #261, #262, #263, #264 | re-verified: **#194** (a `ghp_` token inside a JSX attribute and a `password =` beside `new Map<string, string>()` are both reported, staged, exit 1), **#197** (`max-w-md` → `max-w-sm` in `AntiAliasControls` now fails at 384 against 442 — the discovery replaced the hand-kept list), **#198** (a sentence pasted from `CHROME_TOOLTIPS.history` into `PROJECT_TOOLTIPS` fails `tooltips.test.ts` at the sentence level), **#203** (`SheetSplitContents.tsx:49-52` parses: “deriving it a second way here would be a second definition of where the user is”), **#204** (`bg-ink/60` spelled in a component comment fails `npm run build` from the `closeBundle` hook, naming the file), **#209** (200 code lines appended to `spriteOrdinal.ts` fails `module-size`, whose exempt-path check also fails an exemption that has stopped covering anything). **Closed as already fixed:** **#200** — `76c7fd1` (2026-09-04) replaced “15 of the shipped worker's 18 entries” with a relationship, and it holds: `PRECACHE_SHAPES` is 57, a clean build reports 60, the docblock names the 3 it says are absent. **#189** — saving moved off the Presets tab, so `PresetCard`'s positional stop now indexes the immutable `PRESETS` alone, `presetSearch.ts` fixes the index before filtering, and the Projects view uses `spectrumStopAt(projectStopIndex(id))`, which is the fix shape that issue asked for. **Commented, description drifted:** **#199** (79 call sites across 44 files and 20 disabled-capable across 16, against the 70/39 and 17/13 filed — the gap widened), **#210** (`Modal`, `src/sw.ts` and the secret scanner are now covered; `HistoryEntry`, `HistoryFooter`, `PresetCard` and `TabSwitcher` still are not), **#164** (still true, cross-linked to #222, which names it). Checked against the tree and still true, no action: #98 (Phase 6 already reopened it with the two readings swapped), #163, #172, #187, #191, #195, #201, #202, #205, #218. | **Thirty-eight mutations across thirty suites**, a fresh sample against the first round's thirty-one: all ten guards added to `tests/` since 2026-08-30, and eighteen of the twenty-five colocated suites added with them. Each applied in the worktree, run, and restored before the next, with `git status --short` clean between. **Thirty-three were caught by the suite they were aimed at** — an unlisted `##` section in CLAUDE.md; `noUncheckedIndexedAccess: false`; three planted credentials; the Architecture store count both by editing the copy and by adding a store; a shared sentence typed out instead of imported; a sentence pasted between two cards; a narrowed select wrapper; a banned class in a comment; a bloated module; off-by-ones in `spriteOrdinal`, `numberWords`, `isOnStep`, `sheetToken`, `projectStopIndex`, `componentTotal` and `packLayout`; a `>>> 0` dropped; `declaresNoClothing` de-normalised; a `deepwiki.com` citation; a CELL share past what `SHEET_CELL_PITCH` allows; the auto-tune prologue measured twice a crop; `isTextEntry` losing `checkbox`; the empty-library-pack refusal removed; `contradictionsIn`'s field filter inverted; both word-boundary matchers stripped; undo and redo swapped; a `keydown` listener never removed; `deleteProject` skipping its refusal; `deleteProjectFrom` made a no-op. **Five were not, and three of those are not gaps.** Two are equivalent mutants: `numberWords` losing its `count > 99` bound (any larger count still gives an out-of-range `TENS` index and returns `undefined`), and `planAsDrawn`'s entry filter widened from `!== 'entirely'` to `=== undefined` (no `absentOption` category carries a `'partly'` entry, and the suite asserts exactly that at lines 332–346). The third, `libraryPackSize` ignoring `quantisePresets`, is missed by its own colocated suite and caught by `useLibraryTransferStore.test.ts`, which is coverage distribution rather than a gap. **The remaining two are #261 and #264**, and they are one shape: a suite states a behaviour in its own comment or title, and the assertion beside it tests something that cannot fail when the behaviour is removed. Both were proven against a full `npx vitest run --maxWorkers=3` — 309 files and 8,962 tests green with the mutation in place, twice — and both change the compiled prompt, shown by `diff` over the real `generatePrompt`. **Baseline: 309 files, 8,962 tests green**, before any scratch file and again after the last restore; that is the full-suite green the first round could not observe, its `C:` being full. **CLAUDE.md re-measured and confirmed:** nine `[data-accent]` hues; ten `--color-spectrum-*` stops; `PROSE_COLLISIONS` exactly seven; the raw-hex exemption exactly the six paths named; `PRACTICAL_COMPONENT_CEILING` 43; three threads in `src/workers/`; `--breakpoint-studio` 70rem = 1120px and `--breakpoint-quantise` 76.5rem = 1224px; seven directories under `src/components/`, all named in the separation-of-concerns rule; eight tracked sheets in `test_sprites/`; `src/types/` carrying no function, so its blanket size exemption is honest. **Docs:** all four `docs/todo/` banners and all six under `done/` read, and none claims a completion it does not have; the spec's own preamble already records the fifth view, so the README alone was wrong (#262). **Stale-reference sweep:** every backticked filename in a comment under `src/`, `tests/` and `scripts/` resolved against the tree — eleven do not resolve and all eleven are legitimate (files the app *writes*, another repository's docs, a module deleted by the commit the prose is explicitly about, and the hashed example that the `assets/index-*.css` pattern exists to explain). **Unproven, recorded not filed:** `guidance-sentence-sharing.test.ts:32` says comments are blanked “as the other five sweeps do”, and five files call `codeOnly` in total, so the figure is right only if it counts sweeps rather than files — an interpretation rather than a demonstrable defect. **The three hand-offs to this phase are resolved.** #200 is closed above. The two “nothing under `tests/` checks …” hand-offs from Phases 9 and 10 — distinct accessible names within one repeating list, a pseudo-element’s contrast, and an accessible name on a keyboard-scrollable region — are each the missing guard for a defect **already filed** as #252, #253 and #255, and CLAUDE.md’s “do the whole fix” rule already obliges each of those to ship with a test that would have caught it. Filing a fourth issue saying the guard is absent would report the same root cause a second time, so none was filed. **Confirmed and handed to the wrap-up:** `npx prettier --check docs/todo/codebase-audit-2-plan.md`, which step 3 of the completion checklist names, matches nothing and passes vacuously — `.prettierignore` excludes `*.md`, and `--list-different` on the file exits 0 having listed it nowhere. Correcting the checklist is a rewrite of live guidance rather than of a phase record, which the checklist’s own step 2 reserves. **Out of scope, for the owning phase:** Phase 11 — `npm install` in a fresh worktree rewrote `package-lock.json` by +18/−33 lines, so the committed lock is not what a clean install resolves. **Method note for the next phase that mutates:** two experiments used the same scratchpad backup filename, and a background full-suite run overwrote the clean copy with a mutated one; `git checkout --` recovered it, and no work was lost, but a per-experiment backup name is the fix. **Not driven:** no browser session this phase and no screen reader; #262's reachability claim rests on `ui.test.ts` asserting that `APP_TAB_CHOICES` equals `APP_TABS`, not on a click. |
| Wrap-up | complete | 2026-09-06 | #265, #266, #267, #268 | re-verified: none of this phase's own — it re-checks no closed issue. **#200 commented** (closed, and stays closed): its reported site is genuinely fixed by `76c7fd1`, so the two sites Phase 11 found carrying the same defect are new work in different files by a different commit, which CLAUDE.md's closing rule sends to a new issue linked to the closed one rather than to a reopening — that is #267, with the counts re-measured today rather than carried over. **#263 and #239 commented** to cross-link the two families the set holds. | **No row was `partial`**, and Phase 3B is complete rather than blocked — the maintainer's 27 runs arrived and were scored, so the one row this plan allowed to stay open did not need to. **Eighteen logged suspicions and hand-offs were chased; four proved out and were filed, and fourteen were discarded with the reason.** **Filed (4).** **#265** — `sheetLayout.ts:20` says “The canvas is the largest sprite”; the code takes the deepest **band** a strip occupies and says so in an inline comment 66 lines below. Reproduced through the real function on a two-box strip (canvas 4, tallest sprite 3) and on all eight corpus sheets: `cyborg_healer.png` is 12px deeper than its tallest sprite and `cyborg_black_red.png` 1px, while the *width* column agrees with the bullet on all eight — which is why the sentence reads plausibly, being right about one axis. **#266** — the Flux wrapper's two prompt-length figures. “The specification runs to roughly 3,600” tokens measures **6,529** at the studio's opening `CHARACTER` configuration, and 3,600 is within a rounding of the *smallest* of the thirteen category defaults (INTERFACE, 3,561) rather than of “the specification”; “around token 1,070” measures **1,280**, in two files. Both decisions the figures support survive, since both readings are far past Flux's 512-token ceiling either way. **#267** — `sw.ts:80-82` and `sw.test.ts:11-12` restate the precache split as “39 of the 46” and “thirty-nine”; a clean build emits 60 entries, **58 unique, 51 with `revision: null`, 7 stable URLs**, parsed out of the emitted `dist/sw.js` rather than read off the build summary. The “other seven” half and its list of names are still exactly right. **#268** — `npm install` in a worktree used for nothing else rewrites `package-lock.json` by +18/−33 (`peer` markers on four packages, two bundled `@emnapi` entries) on npm 11.6.2 / node 25.2.1, so CLAUDE.md's `git add -A` commits the rewrite into every unrelated change; `npm ci` is unaffected, which is why CI never reports it. Phase 11 judged this toolchain drift and Phase 12 handed it back — the cost Phase 11 itself named is what makes it a repository defect rather than a machine's. **Discarded, each with its demonstration (14).** §10 on the Sol must-carry list and `nativeGridScale.ts`'s two gates are both ground rule 1: `sol.ts` states the standard (“A run that pins one and loses its limits is what would earn it a place”) and `nativeGridScale.ts`'s third bullet states the conjunction outright, that a sheet cut into parts holds an *assembled* size which “prices a canvas of fifteen whole characters”. Its consequence was filed, as #244 and #245. `GENERIC`'s uncited capabilities: the header's claim is about “somebody else's product” and the comment two lines below the flags says “The one entry with nobody to cite”. `models.ts:334-336` on `stability.ai/brandstudio`: a 404 makes “Stability's own web generator is gone” more true, not less. **Qwen's “about half as long again” does not reproduce as Phase 4 read it** — measured today at 6,552 estimated tokens against 4,500, which is **1.46×**, not the 1.35× that hand-off recorded; the description was written to fix #124 and still holds. `model-citations.test.ts`'s blind spot is stated and argued at `:58-65` (“No sweep can catch that”), and what it let through is #228 and #232. **The `SymmetryControls` duplicate-key warning does not reproduce**: run three ways on `c8f8ee4` — that file alone, the whole of `src/components/quantise/` (22 files, 220 tests), and the full suite with output captured to a file — `grep -icE "same key\|Encountered two children"` returns **0**. A rendered-select guard has no defect behind it: Phase 10 measured all 25 selects at five viewports and every one clears its budget. `guidance-sentence-sharing.test.ts:32`'s “other five sweeps” is an interpretation — five files call `codeOnly`, four besides that one, and two of them run two sweeps each. Phase 2's five-character residue bears on no claim either phase made. Phase 8's projects note claims no defect and Phases 9 and 10 swept those views. `test_sprites/glyph_sprite_sheet.png` is untracked, so it reaches nobody; adding it would change CLAUDE.md's “eight sheets” and `sheetCorpus.ts`'s union, which is the maintainer's decision. The eight leftover `.claude/worktrees/` directories are empty (`find -mindepth 1` returns nothing in all eight), unregistered and git-ignored; left alone. **The set reconciled.** 43 issues are the audit's — #223–#227 are Dependabot pull requests and **#249 was filed by a fix session working #248**, not by a phase, so neither is counted. Every open audit issue carries exactly one `status:` and one `effort:`; the three closed ones carry neither, which is right; #244–#246 sit at `status: needs-design` by the maintainer's re-triage. Each wrap-up issue was deduped against open and closed issues under several phrasings before filing. Nothing describes something a later phase showed to be deliberate — Phase 10 dropped two inherited hand-offs on exactly that ground and Phase 12 re-checked ten open issues against the tree. **Cross-linked by comment rather than by editing a body**, since an edit rewrites a record: **#263** carries the *restated-figure* family, which is the largest cross-cutting root cause either round found — #199, #237, #238, #258, #259, #263, #265, #266, #267 open and #105, #131, #139, #200 closed — with the two fix shapes this repository has already used, state the relationship or give the figure a guard; **#239** carries the download path, #239 / #260 / #265, where #265 matters because `sheetLayout` is the half that orders correctly. **The two rounds compared.** Round one filed **59** (43 now closed) reading a directory and fuzzing it; round two filed **43** (3 now closed, 1 reopened) starting from the behaviour a reader meets. The type mix barely moved — bug 27→26, test 23→21, docs 22→17, content 21→16 — so the second direction found the *same kinds* of defect in a codebase the first round had already been through, which is the plain answer to whether a second round was worth running. **Where it moved is the area mix, and it moved where the plan aimed it.** `prompt-compiler` went 24%→37% of the round and overtook `quantise` (29%→23%); `target-models` nearly doubled its share, 10%→19%, on two whole phases fetching every cited vendor page as source. `common-ui` fell 15%→9% and `design-tokens` 8.5%→4.7% — but round two *drove* those surfaces rather than reading them, and the four it did find (#250–#253) are three measured from rendered pixels or Chromium's own accessibility tree and one from a `<dialog>`'s `cancel` event, which no amount of reading reaches. Fewer defects, and different ones. Two areas are new this round — `identity-lock` (#254) and `dependencies` (#268) — and two appear only in round one, `motion` and `architecture`. **Where neither round found anything is the evidence a third would start from.** `area: atlas-calculator` is empty in both, and it is empty having been swept: Phase 7 put 20,100 configurations through the atlas maths, found no overflow, no impossible share and pricing that matches an independent block count, and reproduced `widthBiasFor`'s and `encodePng`'s own docblock figures exactly. The encoders are the same shape of clean — every PNG, zip and `.aseprite` the app writes read back by Python and by a from-scratch parser, with two documentation defects found and no format defect. A third round should not re-audit either from the same direction; what it has not had is a *reader* driving them, which is what turned up four accessibility defects in surfaces round one had read. **The one class round two invented is the one that cannot be run from this repository.** The six effectiveness findings (#243–#248) rest on 27 real Sol runs the maintainer produced, and three are already fixed and closed (#243, #247, #248) — the fastest any class in either round has moved. The pack's own limit is worth carrying forward: 16 of 27 interrogations returned a refusal, so 371 of 624 hand-off cells are empty and those runs' losses cannot be attributed to the composition or to the render. A third round wanting this class needs transcripts more than it needs sheets. **This document's own checklist was corrected** at step 3, which named `npx prettier --check` on a path `.prettierignore` excludes — vacuous for eleven phases, demonstrated by Phase 12 and reserved for here, because a third round copying the checklist would inherit a check that does nothing. Session prompts still name the pre-archive path, as round one's do: they are records of what was run. Baseline before any scratch file and again after the last was deleted: **309 files, 8,962 tests green**, `npm run build` green. Running notes at `<scratchpad>/audit2-wrapup-notes.md`. |
