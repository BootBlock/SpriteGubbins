...
> **Status:** 🟢 ACTIVE — Phase 7 (downloads, encoders and atlas maths) is complete. Phase 8 (stores, persistence, session and history) is next; Phase 3B (adherence scoring) stays **partial** until the maintainer’s runs exist, and resumes from its own session prompt unchanged.
