...
> **Status:** 🟢 ACTIVE — Phase 6 (quantiser pipeline and its workers) is complete. Phase 7 (downloads, encoders and atlas maths) is next; Phase 3B (adherence scoring) stays **partial** until the maintainer’s runs exist, and resumes from its own session prompt unchanged.
