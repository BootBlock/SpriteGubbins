...
> **Status:** 🟢 ACTIVE — Phase 8 (stores, persistence, session and history) is complete. Phase 9 (UI primitives, chrome, modals, tabs) is next; Phase 3B (adherence scoring) stays **partial** until the maintainer’s runs exist, and resumes from its own session prompt unchanged.
