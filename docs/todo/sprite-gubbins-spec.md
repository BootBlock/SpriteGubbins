...
> **Status:** 📘 REFERENCE — all five phases shipped: build system and PWA shell, design tokens, domain types and option pools, the prompt compiler, SQLite-on-OPFS persistence (in a worker — see the note on §2.4 below) with its localStorage fallback, the Zustand stores Phase 3 lists (since joined by others — see below), the full component tree and app assembly, and the verification pass. Kept here as the durable blueprint the implementation is held to, not as open work.
>
> The places where the implementation knowingly departs from the text below, each verified in a real browser unless the entry says how else it was checked:
>
...
> - **Tasks 2.2 and 2.3, and §4.3's `TargetModelSelector` — the target models are a different roster.** The text below names `GOOGLE_IMAGEN_3` and `DALLE_3`, with an Imagen 3 prefix and a DALL-E 3 prefix, and pins Midjourney at `--v 6.1 --style raw`. Neither Imagen nor DALL-E is a target. The roster in `TARGET_MODELS` (`src/constants/models.ts`) keeps Generic, ChatGPT 5.6 Sol, Midjourney and Stable Diffusion, and adds two Gemini image models, Seedream, Qwen-Image, two Flux targets (open weights and the BFL API) and GPT Image. Midjourney is pinned by `MIDJOURNEY_VERSION` and takes `--raw`, which is the raw-mode flag the vendor's current pages give. Each entry that names a model is checked against its vendor's own documentation rather than in a browser, and the comments beside it cite the pages or record where a vendor publishes none — Generic names no model, and Stable Diffusion's weights have no vendor page, so its prompt syntax cites a front end's. Do not restore a target this text names and the roster dropped, or treat one it adds as unauthorised: the roster follows the generators a reader can use today.
>
> The app has since grown past this blueprint, and the places it has are recorded here so a reader is not left comparing the tree against a document that predates them. Four of them — the `quantise` view, reading images, the sheet splitter and the component budget — came off [baseline-prompt-new.md](baseline-prompt-new.md) §10's follow-up list rather than the phases above, and that list is now closed; the rest is growth §10 never anticipated either:
>
> - **A fifth view, `projects`**, and the container every saved preset now belongs to. This document's §2.4 stores a reader's own archetypes as one flat collection, which is right for a handful and unusable for somebody working on two games at once: a search is through both, and nothing says which preset was for which. A project is a name, a sentence and a **GUID that never changes**, in a `projects` table, and both saved collections carry a `project_id` referring to it — so a project can be renamed freely without any reference going stale. Three things follow that the phases above do not describe: saving moved off the Presets tab, which is now the built-in library alone, and into the Studio and the Quantise tab beside the configuration each of them saves; deleting a project deletes what is filed in it, in one transaction; and the two preset packs became one **library pack** carrying the projects and both collections together, because a preset naming a project the file does not hold describes a library that cannot be assembled. Every save is filed under the `Default` project until the reader chooses otherwise, and that project cannot be deleted.
...
> - **A component budget** on `OutputConfig`, reported when a sheet outgrows it — against the sheet the studio is configured to, and on each row of the split drawer, because the sheets of a series carry different parts of one inventory and only one of them may be over — and deliberately absent from the compiled prompt.
> - **A preset says what it is for.** `PresetArchetype` carries a `description` beside the five fields Task 2.1 lists — a sentence the card leads with, the search matches on, and the save panel lets a reader write. A built-in library that runs to scores of presets is not navigable by name and settings alone: neither says why you would reach for one. It is required on the type and empty only on a preset of the user's own, so `custom_presets` carries a matching `NOT NULL` column that §2.4's table does not describe.
> - **A machine to draw for, and a palette to draw it in** — two further `OutputConfig` fields this document never contemplated, in `src/constants/hardware/` and `src/constants/palettes/`. The first is the Output Configuration panel's template dropdown: choosing the Game Boy, the Mega Drive, the C64 or one of fifteen others writes the render settings that machine's artwork actually used, and puts its display, tile grid and sprite limits into §2 of the prompt. The second makes palettes first-class — a fixed list of colours or a machine's bits-per-channel colour space — which the prompt states in full and the Quantise tab maps a returned sheet onto, superseding the `PALETTE_LIMIT` budget §2 of this document specifies wherever one is pinned. Its `CUSTOM` member is the reader's own colours rather than a machine's, carried on `OutputConfig.customPalette` and loaded from the three forms a palette leaves this app as — a swatch picture, a `.gpl` or a pasted hex list. Once loaded it behaves as a machine's list does everywhere, and a preset or a restored history row carries it, so the row recompiles the prompt it recorded.
> - **The artwork leaves as assets, not only as a picture.** The Quantise tab's download offers four formats where this document contemplates none: the sheet as an indexed PNG, an `.aseprite` document, a **sprite pack** — a ZIP holding the sheet, one PNG per piece the sheet resolved to, and a manifest — and that **manifest** on its own as JSON. The manifest states each sprite's rect and duplicate link in the written file's own pixels, and names the pieces from the prompt's own inventory: section 4 fixes the reading order, so the *n*th sprite is the *n*th component. Every inventory line therefore carries a `label` and, where its components are told apart by what they are rather than by where they sit, a `parts` list naming each of them (`src/types/components.ts`) — which `componentSlots` expands into one name per component, so a fifteen-piece character rig cuts out as `04-left-upper-arm.png` rather than `04-left-arm-1.png`. It states a pivot too, and says which it is: the bottom-centre default rather than a measurement, or the anchor the reader named where they asked for a cell, and the sheet record carries the rig that was asked for so a consumer can tell whether that default is a starting point or a number to replace — together with the whole rig contract on the sheet that draws the rig’s pieces, wherever one was loaded, so an importer can refuse a pack drawn against a different rig, or against an older revision of one, rather than place every piece in the model’s own proportions and report nothing. The two describing formats also cut every sprite into a **fixed cell** rather than into its own bounding box, at that stated anchor, because a rig importer of cut-out pieces declares one slot size per piece and refuses artwork that does not match it (`src/types/spriteCell.ts`); the cell is a canvas the sprite's box is laid on rather than a wider cut of the sheet, and a sprite larger than it refuses the download rather than being resampled. The names are attached only where every inventory name is taken by exactly one piece, and the reader decides what a piece is: the Sprites panel lists every sprite the sheet broke into, and each can take an inventory name, be left out of the download, or be joined to another so the two are written as one piece. The preview labels each sprite with the name it will be written as, before any download. A count alone could not make that judgement — a sheet that drew the right arm before the left has the right number of pieces and the wrong name on both — so the manifest records which of the two routes named it, the reading order the prompt fixes or the reader's own assignment.
> - **The prompt can be written against an engine's own rig.** `OutputConfig` carries a `rigContract` — a versioned JSON document an engine's rig tooling exports, loaded as a local file in the studio and read by `parseRigContract`. Where one is loaded on the sheet whose inventory *is* a rig, section 4 lists the engine's own piece names, section 5 states each piece's size within the assembled frame along with its joint end and pivot, and section 2's assembled size and the native grid come off the frame rather than off a size typed by hand. It is the first field in this document's §2.2 configuration that the reader does not compose: every other one is a choice between values this app offers, and this is a measurement another program published. Nothing is uploaded — the file is read in the tab, as an image is. A contract also reaches the configuration without a file: the built-in `Unsung Saviour — Character rig` preset ships a transcription of that game's exported document, so that preset compiles a prompt carrying the geometry, and so applying *it* does not silently clear a contract the reader had loaded — every other preset carries none and does clear one, which is what a preset replacing the whole configuration means. The shipped copy cannot know when its rig moved, so `isShippedRigContract` tells the two apart under the control and an export loaded over it wins. Plan: [done/rig-contract-import.md](done/rig-contract-import.md).
> - **A Settings dialog**, opened from the header's far right — another overlay §4.4 never lists. It holds the reader's preferences about the app itself rather than about a sheet: the accent colour, motion, the ambient backdrop and the view the app opens on. `useSettingsStore` keeps them in an `app_settings` table that §2.4's DDL does not describe, apart from `useUIStore`, because a preference outlives the tab and the shell state is meant to be forgotten with it.
> - **More stores than Phase 3 lists, one per concern.** Phase 3's stores are all still there, and the growth recorded here brought its own: projects, quantisation with its Auto tuning and its saved dials, the Sprites panel, sheet downloads, the library pack, which collapsible sections are open, the settings, and the studio session — the category, subject and output configuration kept between visits in a `studio_session` table, so closing the tab no longer loses a configuration nobody saved. `src/stores/` is the list; the saved dials are the `quantise_presets` table, which §2.4 does not describe either.
