...
> - **The artwork leaves as assets, not only as a picture.** The Quantise tab's download offers four formats where this document contemplates none: the sheet as an indexed PNG, an `.aseprite` document, a **sprite pack** — a ZIP holding the sheet, one PNG per separated sprite, and a manifest — and that **manifest** on its own as JSON. The manifest states each sprite's rect, pivot and duplicate link in the written file's own pixels, and names the pieces from the prompt's own inventory: section 4 fixes the reading order, so the *n*th sprite is the *n*th component. Every inventory line therefore carries a `label` (`src/types/components.ts`), which `componentSlots` expands into one name per component. The names are attached only where the sheet's sprite count matches what the prompt asked for, and the same comparison is reported in the Sprites panel — the reading this app could not previously make about a returned sheet. The two describing formats also cut to a **fixed cell** rather than to each sprite's own bounding box, at a stated anchor, because a rig importer of cut-out pieces declares one slot size per piece and refuses artwork that does not match it (`src/types/spriteCell.ts`); a sprite larger than the cell refuses the download rather than being resampled.
