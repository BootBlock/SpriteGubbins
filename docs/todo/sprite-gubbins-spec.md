...
> - **The artwork leaves as assets, not only as a picture.** The Quantise tab's download offers four formats where this document contemplates none: the sheet as an indexed PNG, an `.aseprite` document, a **sprite pack** — a ZIP holding the sheet, one PNG per separated sprite, and a manifest — and that **manifest** on its own as JSON. The manifest states each sprite's rect and duplicate link in the written file's own pixels, and names the pieces from the prompt's own inventory: section 4 fixes the reading order, so the *n*th sprite is the *n*th component. Every inventory line therefore carries a `label` and, where its components are told apart by what they are rather than by where they sit, a `parts` list naming each of them (`src/types/components.ts`) — which `componentSlots` expands into one name per component, so a fifteen-piece character rig cuts out as `04-left-upper-arm.png` rather than `04-left-arm-1.png`. It states a pivot too, and says which it is: every one is the bottom-centre default rather than a measurement, and the sheet record carries the rig that was asked for so a consumer can tell whether that default is a starting point or a number to replace. The names are attached only where the sheet's sprite count matches what the prompt asked for, and the same comparison is reported in the Sprites panel — the reading this app could not previously make about a returned sheet.
