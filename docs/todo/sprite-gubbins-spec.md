...
> - **A preset says what it is for.** `PresetArchetype` carries a `description` beside the five fields Task 2.1 lists — a sentence the card leads with, the search matches on, and the save panel lets a reader write. A library of seventy is not navigable by name and settings alone: neither says why you would reach for one. It is required on the type and empty only on a preset of the user's own, so `custom_presets` carries a matching `NOT NULL` column that §2.4's table does not describe.
