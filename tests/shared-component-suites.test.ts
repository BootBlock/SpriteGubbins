 ...
 * least tested code in the app that is not data. Five defects went through exactly those files
 * before this guard existed (#210 lists them): focus dropped to `<body>` by a dialog closing after
 * React had detached it, and by five confirm-in-place presses. Each is a focus assertion after a
 * press, which is what `user-event` is for, and none had a suite to be written in.
 *
 ...
 * at once; a module in `layout/` is on screen in every view there is. A panel under `studio/` or
 * `quantise/` has one call site and one view, and is not held to this.
 *
...
/**
 * The parts: a module split out of a component for the file-size law, whose behaviour its parent's
 * suite asserts.
 *
 * **A part is not a second primitive.** It has no call site of its own outside the component it was
 * lifted from, so a suite for it would render it without the state that gives it any behaviour — the
 * card with no trigger, the option with no list — or duplicate the parent's cases one for one. The
 * guard still checks the claim each entry makes rather than trusting it: the part has no suite of its
 * own, every module importing it is one of its named parents, and each of those has a suite.
 *
 * **Not a list of the untested.** A module earns a place here by being part of one component, never
 * by being small or declarative. `SectionToggleAll` looked like a part, with its cases in
 * `CollapsibleSection.test.tsx`, and was not one: two panels render it beside their disclosures, so
 * those cases moved to a suite of its own.
 */
...
/** Whether `importer` imports `module` through a relative specifier. */
function imports(importer: string, module: string): boolean {
  ...
  return sourceText(resolve(process.cwd(), importer)).includes(`from '${specifier}'`);
}
  ...
  it('renders the component each suite is named for', () => {
    const unrelated = sharedModules().flatMap((module) => {
      ...
      return suite === undefined || imports(suite, module)
        ? []
        : [`${suite} does not import ${basename(module)}`];
    });
