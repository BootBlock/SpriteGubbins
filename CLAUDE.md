...
| `forEach(async …)`, floating promises | `@typescript-eslint/no-floating-promises`, `no-misused-promises`, `await-thenable`, on **every** TypeScript file `eslint .` lints — `src/` and the `vite.config.ts` / `scripts/` / `tests/` program alike. `tests/type-aware-lint-scope.test.ts` asks ESLint what each file resolves to, and fails if any rule declaring `meta.docs.requiresTypeChecking` is missing from one of them or set differently there. That flag is typescript-eslint's convention, so a type-aware rule from a plugin that omits it is outside the check. |
| Unsafe array indexing (`array[0].prop`) | `noUncheckedIndexedAccess`, set in **both** tsconfigs — `src/` and the `vite.config.ts` / `scripts/` / `tests/` program alike. `tests/tsconfig-strictness.test.ts` fails if either config drops a strictness flag the other sets. |
