...
export function DownloadControls({
  ...
  useEffect(() => {
    ...
    // sees that; a keyboard user loses their place in the toolbar for as long as the file takes. It
    // is restored only where nothing else has claimed focus in the meantime, so a reader who moved
    // on during the write is not pulled back.
    if (wasWriting.current && !download.saving && document.activeElement === document.body) {
      button.current?.focus();
    }
    wasWriting.current = download.saving;
  }, [download.saving]);
