...
export function PresetCard({ preset, index, onLoad, onRename, onDelete }: PresetCardProps) {
  ...
  return (
    <li
      ...
      className="animate-pop-in glass-panel group relative flex flex-col justify-between gap-4 overflow-hidden rounded-2xl border border-tab/35 p-5 shadow-xl transition-all duration-450 hover:-translate-y-1 hover:border-tab/80 hover:shadow-2xl"
    >
      {/* A bloom in the card's own colour, existing only under the pointer, behind its content. */}
      <span
        aria-hidden="true"
        className="pointer-events-none absolute -top-16 -right-16 size-32 rounded-full bg-tab/25 opacity-0 blur-2xl transition-opacity duration-750 group-hover:opacity-100"
      />
      ...
      <div className="relative space-y-3">
        ...
        {isRenaming ? (
          ...
          <h3 className="text-base font-bold text-ink transition-colors duration-450 group-hover:text-tab">
            {preset.name}
          </h3>
        )}
      ...
      {isConfirmingDelete ? (
        ...
        <div className="relative flex gap-2">
          <button
            ...
            className="action-tab group/load flex flex-1 items-center justify-center gap-1.5 rounded-xl py-2 text-xs font-semibold transition-all duration-300 active:scale-[0.98]"
          >
            {/* Named group: the card is already a `group`, and an unnamed one here would follow the
                card's hover rather than this button's. */}
            <span
              aria-hidden="true"
              className="inline-block transition-transform duration-450 group-hover/load:scale-125"
            >
              ⚡
            </span>
            Load preset
          ...
          {preset.isCustom === true && (
            <>
              <button
                ...
                className="rounded-xl border border-foundry-600 px-3 py-2 text-xs font-semibold text-ink-muted transition-all duration-300 hover:-translate-y-px hover:border-tab/50 hover:bg-foundry-700 active:translate-y-0"
              >
              ...
              <button
                ...
                className="rounded-xl border border-foundry-600 px-3 py-2 text-xs font-semibold text-rose transition-all duration-300 hover:-translate-y-px hover:border-rose/50 hover:bg-foundry-700 active:translate-y-0"
              >
