...
interface PresetDetailsFormProps {
  ...
  /** Close without saving. The row uses this to put focus back where it came from. */
  readonly onClose: () => void;
}

/**
 * Editing one saved preset's name and the sentence under it, in place of its heading.
 *
    ...
    <form
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          if (await updateCustomPresetDetails(preset.id, draftName, draftDescription)) onClose();
        } finally {
          setIsSaving(false);
        }
      }}
