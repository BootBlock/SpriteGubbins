 ...
 * can have nothing to answer with. The empty field is the obvious one. The other is a sheet whose
 * components are the pieces one subject is cut into — a cut-out rig, a pose library, an ITEM part
 * library. Such a sheet states its size for the assembly, so the studio hands this panel no
 * component size even though the field in front of the reader is full. Saying "no component size named" over `48 × 96 px assembled` reads as
 * the panel having failed to see it, which is a worse answer than the wrong number it replaced.
 *
 * **The two are told apart by the sheet, not by the field.** Driven the other way, such a sheet with
 * the box empty got the branch that asks for a size — promising that naming one turns this into a
 * check, which on that sheet it never will: its components are pieces of whatever the size
...
describe('AtlasFitSummary with no component size to check', () => {
  ...
  it('says why there is nothing to check where the size names the assembly', () => {
    ...
    expect(screen.getByText(/the pieces one subject is cut into/u)).toBeInTheDocument();
  });
});
