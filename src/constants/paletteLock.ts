...
export const PALETTE_LOCK_GUIDANCE = {
  ...
  held: 'Each colour in this sheet that sits within the snap distance of a held colour is taken to it, measured the way every colour distance on this tab is. Anything further away keeps the colour it arrived with, so a gem or a flame the locked sheet never had is not flattened into it — which also means a lock does not fix how many colours come out. The sheet-wide colour merge is left off while a palette is held, since folding two held colours together here would quietly edit the palette the rest of the series is mapped onto, and the studio’s colour budget does not run either.',
  /**
