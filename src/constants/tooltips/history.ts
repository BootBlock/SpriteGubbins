...
export const HISTORY_ACTION_TOOLTIPS = {
  ...
  restoreEntry:
    'Puts the studio state this prompt was compiled from back into the studio — the category, every subject field and every output setting — so you can change something and recompile rather than editing prompt text by hand. Whatever is in the studio now is replaced, and recorded in the Subject history panel on that tab first, so one Undo brings it back. An entry whose stored studio state can no longer be read restores to its category’s defaults instead.',
