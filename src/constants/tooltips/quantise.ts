...
export const QUANTISE_ACTION_TOOLTIPS = {
  ...
  autoTune:
    'Sweeps the dials that decide how this sheet is read, and puts them where the sheet itself says they belong. It runs the whole pipeline over three busy crops of your image at around sixty combinations of the cell reading, the outline expansion, the ink blend, the ink threshold, the colour merge, the fill cleanup and the cleanup passes, and keeps the one that reproduces those crops most closely for the fewest colours. Those eight dials are all it moves: the pixel scale, the background keying, the dither, the palette snap, the sprite gap and the symmetry and duplicate settings stay exactly where you put them, because none of them is a question a likeness score can answer — keying deletes pixels, a dither trades likeness away on purpose, and the rest change what the tab reports rather than what it draws. The whole answer lands as one change, so a single undo puts every dial back where it was. Read it as a starting point rather than a verdict: it judges crops rather than the whole sheet, and your own eye on the preview is what settles it.',
  candidateFromSheet:
