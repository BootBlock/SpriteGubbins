...
export const QUANTISE_ACTION_TOOLTIPS = {
  ...
  downloadPNG:
    'Saves the quantised sheet as a PNG, at its own true size rather than at the magnification the preview is showing. That size is the sheet divided by the pixel grid, so a 1024 px sheet read at a grid of 8 is written out 128 px across — genuine pixel art, one file pixel per drawn pixel, which is what an engine wants to import. Transparency is kept where the background has been keyed out. It is unavailable until a grid is settled, since there is nothing to write until then.',
