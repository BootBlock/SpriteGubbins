...
export const QUANTISE_ACTION_TOOLTIPS = {
  ...
  downloadPNG:
    'Saves the quantised sheet as a PNG at the magnification the Save At control states, whatever the preview is showing. At 1× that is the sheet’s own true size — the sheet divided by the pixel grid, so a 1024 px sheet read at a grid of 8 is written out 128 px across, genuine pixel art with one file pixel per drawn pixel, which is what an engine wants to import. Where the result holds 256 colours or fewer it is written as a true indexed PNG, carrying its palette in the file rather than only in this panel, which is the form a game pipeline, a tile editor or a palette-swap shader reads; a sheet with more colours than that is written the ordinary way instead, and the confirmation says which you got. Transparency is kept where the background has been keyed out. It is unavailable until a grid is settled, since there is nothing to write until then.',
