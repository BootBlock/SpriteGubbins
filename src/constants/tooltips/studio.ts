...
export const STUDIO_ACTION_TOOLTIPS = {
  ...
  undoSubject:
    'Puts the subject back the way it was before your last category switch, Randomise, Reset, preset load or restore from the prompt history, or before an assembly base moved your sheet — the whole studio as it stood then, Output Configuration included, because each of those can move those settings along with the answers. Those acts are the only things recorded, because typing a value back cannot undo them; editing a field records nothing otherwise, and an edit you made after one of them is not lost either, because Redo brings the studio back exactly as you left it. Nothing outside the Studio tab moves. Ctrl+Z does the same thing, except while you are typing in a box, where it undoes your typing as it always has, and while a dialog is open, where the page behind it is not what your keyboard is aimed at.',

  redoSubject:
    'Steps forward again into a subject you have just stepped back from, which is the way out of an undo pressed once too often. Switching category, randomising, resetting, loading a subject from anywhere or choosing an assembly base that moves your sheet after stepping back replaces what was ahead of you, so this is offered only until you do. ' +
  ...
  loadRigContract:
    'Reads a rig contract your engine exported — the file its rig tooling writes — and takes the sheet’s piece list from it. Section 4 then lists each piece under the name the engine itself uses, so the pack you cut afterwards needs no renaming, and section 5 states every piece’s size within the assembled frame, which end of it the joint is at, and where that joint sits on the figure. Without it the prompt can state only the assembled size, and each piece comes back at whatever share of the figure the model chose. The file is read in this tab and never leaves it, and it is carried with the rest of the configuration, so a saved preset or a restored prompt keeps it. A preset may also bring a contract with it, and the line under this control says when the one in force came that way rather than from a file of yours.',
