...
export const ARCHITECTURE_SECTIONS: readonly ArchitectureSection[] = [
  ...
  },
  {
    ...
    body: 'The compiled prompt is a pure function of the category, the sixteen subject fields and the thirty output settings: the same state always produces the same text. A field you leave blank omits its line entirely rather than filling it with a placeholder, and the prompt says outright that an absent attribute is the generator’s to choose. Each target then gets what its own documentation calls for — a reasoning directive for ChatGPT 5.6 Sol, command flags for Midjourney, a weighted negative-prompt block for Stable Diffusion and an unweighted one for Qwen-Image, the same constraints restated up front for Flux (which takes no negative prompt, and whose open weights stop reading at 512 tokens), and a note to Seedream about what to sacrifice from a brief it is documented to drop details out of. GPT Image, the Gemini image models and the Generic option add nothing, deliberately: OpenAI document no prompt rewrite for the GPT image models on the Images API, and the Gemini models read the prompt as a specification already. The template itself also adapts — a target with no pass in which to check its own work is not sent the self-audit, and only a target that returns text is asked for a component map.',
  },
  {
  ...
  },
  {
    ...
    body: 'Choose one of eighteen systems — seventeen real machines, the Game Boy, the NES, the Super NES, the Mega Drive, the Master System, the Atari 2600, the C64, the Spectrum, the Amiga, the Neo Geo and the rest, and one fantasy console, PICO-8 — and the studio fills in the render settings that machine’s artwork actually used, while the prompt states its display, its tile grid where it had one, and, for seven of the consoles, how many sprites may cross one scanline. Colour is a separate setting, so the two can be mixed: a palette is either a fixed list of colours the prompt writes out in full, or the bits-per-channel space the machine defines, from 64 to 262,144 colours, and it supersedes the general colour budget wherever it is set. It can also be your project’s own colours rather than a machine’s — load them from a swatch picture, a .gpl or a pasted list of hex, and the prompt names them the same way. The Quantise tab then maps a returned sheet onto that exact palette rather than choosing colours of its own.',
  },
  {
    heading: '6. Browser-embedded SQLite',
    body: 'Your prompt history, saved presets and interface settings are stored in a real SQLite database compiled to WebAssembly, persisted to the Origin Private File System. Where that is unavailable — a private window, a browser without OPFS, an exhausted quota — the same interface is served from your browser’s local storage instead, so your work still stays in this browser. A browser that will not let the app read its local storage either gets a store in this tab’s memory, which keeps nothing once you close or reload the tab, and the status above says so. A second tab is deliberately served neither way: the database admits a single writer, so a second tab of this app cannot open the one the first tab holds. That tab reads nothing and saves nothing rather than starting a library of its own, and says so above — close the other tab and reload to reach your work.',
  },
  {
