 ...
 *
 * The thirteen `DEFAULT_*` constants in `quantiser.ts` each say what one control opens at and why.
 * This is the set they form, and it exists because three separate things now need to name *all* of
 * them at once: what `useQuantiseStore` opens with, what its `clear` puts back, and what
 * `parseQuantiseTuning` falls to field by field when stored text cannot be read.
 *
 * Typed as {@link QuantiseTuning} rather than inferred, which is the point of writing it down: a
 * dial added to that interface fails to compile here until it has an opening position, and fails in
 * the parser until it has a check.
 *
 ...
 * DOM library, deliberately. `QuantiseTuning` reaches `types/quantiser.ts`, which is written in
 ...
 */
export const QUANTISE_DEFAULT_TUNING: QuantiseTuning = {
  keyingEnabled: false,
