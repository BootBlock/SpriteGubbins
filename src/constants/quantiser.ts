import { CHECK_CHANGES_NOTHING } from './guidanceSentences.ts';
 ...
 * scale holds both of.
 *
 * **A feature against an edge is the shape that can flatter a coarse scale, and no ceiling is what
 ...
 * boundaries inside a sprite and cannot move a silhouette; `SILHOUETTE` softens the outer edge,
 * which means writing partial transparency; `BOTH` does the two. See `AntiAliasMode` for what the
 * first of those guarantees exactly.
 */
...
export const QUANTISE_TOOLTIPS = {
  grid:
    'How many image pixels wide one drawn pixel is. Art drawn at 8 only changes colour every 8 pixels, so that is the scale reported.\n\n' +
    'Where resampling has softened those changes away, a scale is estimated instead and offered for you to click rather than applied. Type one yourself when no reading found a scale, or when the reported one disagrees with the preview.\n\n' +
    'Art inset from the image’s corner needs no cropping: where the grid sits on the art is measured separately whenever a scale is applied. A grid of `1` leaves the size alone and only reduces the palette.',
  previewMode:
    'Which of five ways the result is shown. It changes only what this panel draws; the result, the download and everything stored are the same in all five.\n\n' +
    '- Side by side shows the two frames on the same part of the sheet at the same magnification.\n' +
    '- Wipe lays them over one another under a divider you can drag, so you see the same screen pixels before and after.\n' +
    '- Difference maps what the reduction cost, one mark per drawn pixel: dark where it is faithful, green then gold as it drifts, red where it lost what it replaced.\n' +
    '- Sprites draws a dashed box around each separate piece of artwork found on the keyed sheet.\n' +
    '- Onion skin lays every frame of each row over the first, so a frame that wandered shows as a ghost.',
  differenceScale:
    'How large a difference must be to reach the top of the heatmap’s ramp, measured the way every colour tolerance on this tab is. It changes only the heatmap.\n\n' +
    'Lower rungs grade the small differences most of a sheet is made of, and paint anything coarser in red. Higher rungs flatten those to dark and keep the ramp for edges, and for keyed silhouettes where transparency landed differently from the artwork.\n\n' +
    'The scale is fixed rather than fitted to each sheet, so you can compare the map before and after you move a dial.',
  wipe:
    'Drag this to move the divider between the sheet as it arrived, on its left, and the quantised sheet, on its right. Both are drawn at the same magnification on the same part of the sheet, so the pixels either side of the divider are the same pixels before and after, and a change of a single shade is easy to find.\n\n' +
    'Once it has focus, the arrow keys move it too. Dragging anywhere else in the frame pans both images together.',
  ...
  zoom:
    'How many screen pixels one image pixel is drawn as. Magnifying never resamples: one pixel becomes a square of them. Both previews stay on the same part of the sheet, so moving one moves the other.\n\n' +
  ...
  keyTolerance:
    ...
    'Raise it until the field goes, and stop before the sprite does. If your artwork is a pale version of the key colour, exact leaves it alone.',
  silhouetteThreshold:
    'How much of a pixel must be covered for it to count as artwork. It clears every partly transparent pixel below the setting and makes every one above it solid, so a soft outline comes back as a hard edge one pixel wide.\n\n' +
  ...
  outlineExpansion:
    'How far a drawn line is thickened before the sheet is read down to pixels. A contour one pixel wide is a minority in the patch it crosses, so outlines come back broken. This grows dark ink on light art and bright trim on dark art, so a line keeps enough of its patch to survive.\n\n' +
    'It applies whichever reading the Downscale control has in force. It leaves transparency alone and adds no colour the sheet did not already contain.\n\n' +
    'Off leaves the sheet as it arrived. Raise it when contours come back dashed (`1` is enough on a typical sheet), and back off when fine detail starts to close up.',
  lineStrength:
    'How hard the ink-weighted reading pulls a patch toward the line crossing it. At `1×` a line darkens its patch only by the share it holds; higher settings give more defined contours and thicker-looking darks.\n\n' +
    'It appears only while Downscale is set to `INK_WEIGHTED`. Slide it up when outlines look faint, and back when dark areas swallow detail.',
  trimStrength:
    'How hard the ink-weighted reading pulls a patch toward a bright trim crossing it: the highlight counterpart of line strength, for gold edging and rim light. Where a dark line and a trim cross one patch, the line takes it.\n\n' +
    '`0` leaves highlights to the plain blend, the safer start, because generators often bloom highlights outward. Shown only with `INK_WEIGHTED` chosen; slide it up when bright edging fades out of the result.',
  inkThreshold:
    'How dark a pixel must be before the ink-weighted reading counts it as line ink. The default is the darkest quarter of the tonal range, shared with the line rescue of `DOMINANT`.\n\n' +
  ...
  fillCleanup:
    ...
    'It changes colour only, never transparency, and linework sits far outside its range. Off leaves the result as the reading made it.\n\n' +
    'On a densely dithered sheet, raise the colour merge first, then raise this until stray pixels go; back off if close shades begin to fuse.',
  cleanupPasses:
    'How many times the fill cleanup runs over its own output. A pixel two deep in a speckled patch settles only after its neighbour has, which the next pass picks up.\n\n' +
    'Each pass stops early when nothing changed, so a high setting costs nothing on a sheet that settles quickly. It does nothing while the fill cleanup is off.',
  paletteSnap:
    'How near a held colour a colour in this sheet must sit to be taken to it, measured the way every colour distance on this tab is. Anything further keeps its own colour, so the lock does not flatten a gem or a flame the locked sheet never had.\n\n' +
    'Off means the lock reaches nothing, and the studio’s colour setting decides as if no palette were held. The default leans towards keeping, because a colour kept by mistake costs one extra colour, while one taken by mistake changes the artwork.\n\n' +
    'Raise it when a shade that should have matched comes through as its own; lower it when a new colour is swallowed by the palette.',
  dither:
    'How the palette step spreads a colour the palette cannot hold across neighbouring pixels, instead of rounding each one to the nearest entry. The pattern is fixed to pixel position, so one colour lands on the same pattern in every frame and an animation does not crawl.\n\n' +
    '- `NONE` rounds every pixel on its own.\n' +
    '- `BAYER_4` and `BAYER_8` are the classic ordered crosshatch; the larger carries more mixing ratios.\n' +
    '- `BLUE_NOISE` spreads the same ratios with no repeating figure, the quieter choice where a crosshatch would read as texture.\n\n' +
    'It is offered only while a colour budget, a pinned palette or a locked palette is in force.\n\n' +
    '**A pattern switches off the outline rescue of `DOMINANT`.** Raise the outline expansion, or choose `INK_WEIGHTED`, if contours start to break up.',
  spriteGap:
    'How far apart two pieces of artwork may sit and still count as one sprite, in drawn pixels. A subject rarely comes back as one shape (a sword held clear of the hand, a shadow under the feet), so close pieces are read as one thing.\n\n' +
    '`0` is not an off position: pieces whose boxes overlap are still gathered. Raise it when one subject is counted as several; lower it when two neighbours are counted as one. Past the width of the gutter, the whole sheet folds into one box.\n\n' +
    'It changes no pixel of a PNG, but an Aseprite document, a sprite pack and a manifest are built from these boundaries. With Symmetry or Frame alignment on `SNAP`, or the duplicate snap on, it can change the sheet itself. Switch the preview to Sprites to see where the boundaries fall.',
  symmetry:
  ...
  frameAlignment:
    ...
    '- `OFF` skips the alignment reading.\n' +
    `- ${CHECK_CHANGES_NOTHING}\n` +
    '- `SNAP` also moves each frame that drifted further than the tolerance below back onto its slot, without altering the artwork or reaching a neighbouring sprite.\n\n' +
  ...
  antiAlias:
    ...
    '- `INTERIOR` softens only boundaries between two solid pixels, so no silhouette moves.\n' +
    '- `SILHOUETTE` softens the outer edge, writing partial transparency that changes the bounds of every sprite.\n' +
    '- `BOTH` does the two.\n\n' +
    'It changes the result and the download, never the prompt or anything stored. Softening the outer edge adds colours, and a sheet past a few hundred is saved in full colour rather than as a palette.',
  antiAliasThreshold:
    'How different two neighbouring pixels must be before the edge between them counts as a contour worth softening, measured the way every colour distance on this tab is. Below it, a boundary is treated as two shades of one region and left alone.\n\n' +
    'Raise it when the pass smears the shading steps inside a fill; lower it when a visible contour stays a staircase. `0` is the loosest setting, where every boundary counts; the Anti-aliasing control is what switches the pass off.',
  antiAliasStrength:
    'How much of the reconstructed coverage is written, as a percentage. At `100` each pixel is blended by exactly the fraction the intended edge covered.\n\n' +
    'Lower settings keep the fringe in the same places but fainter, which suits a small sprite, where a heavy fringe reads as noise. To turn it off, use the Anti-aliasing control.',
  antiAliasRun:
  ...
  presetName:
    'What this set of dial positions is called. Name it for what it suits (the generator whose sheets need it, or the style of artwork) rather than the settings, which are already on screen.\n\n' +
    'Names only have to be unique inside one project: a name the chosen project already holds updates that set.',
  presetDescription:
    'An optional sentence shown under the name, for what the name has no room to say: which sheets these settings suit, or what to check after loading them. It has no effect on the sheet or the dials, and you can leave it empty.',
  downloadFormat:
    'Which file the sheet is saved as. None of the four changes the previews, the prompt or anything stored.\n\n' +
    '- PNG is one picture of the whole sheet, which an engine importer, tile editor or asset pipeline reads.\n' +
    '- Aseprite is an editable document: each sprite becomes a frame, each row a tag, and the palette is the document’s own.\n' +
    '- Sprite pack is a ZIP of the sheet, one PNG per sprite and a manifest naming them.\n' +
    '- Manifest is that description alone, as JSON, for a pipeline that already has the sheet.\n\n' +
    'The three that carry pixels write the same pixels and keep the palette where the colours fit one. A sheet with nothing separable on it is still written whole.',

  spriteCellSource:
    'What every sprite in a pack or manifest is cut into. It changes only the file you save.\n\n' +
