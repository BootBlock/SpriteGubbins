...
export const QUANTISE_TOOLTIPS = {
  ...
  antiAlias:
    'Whether the staircase edges this tab produces are softened with anti-aliasing, and how much of the sheet that reaches. Every pass ahead of this one exists to turn a smooth resampled render back into flat pixels, which leaves each contour a run of square steps, and a shallow slope drawn that way reads as a staircase rather than as a line. This pass reads the step pattern back, works out where the intended edge really ran and how much of each pixel it covered, and writes that coverage as a blended pixel — the same thing a pixel artist does by hand with a few intermediate shades. A clean 45° line is left alone, because the reconstruction cuts exactly as much off one side of those pixels as the other, and that is the one contour hand pixel art does not soften either. INTERIOR keeps the softening to boundaries between two solid pixels, so no silhouette moves — nothing is cleared and nothing that was clear gains coverage, and on the ordinary sheet, whose pixels are either solid or clear, no transparency changes at all. SILHOUETTE softens the outer edge instead, which does write partial transparency and changes the bounds every sprite reading and every atlas cell is measured from. BOTH does the two. It is off by default because it adds pixels you drew nothing of, and because how a soft outer edge reads depends on a background this sheet does not contain. The two positions that soften the outer edge raise the colour count by one entry per shade per coverage, and a sheet that passes a few hundred colours is written with its colours in full rather than as a palette — so a sheet you reduced to a machine’s fixed shades downloads as a palette file with INTERIOR and as a full-colour one with the other two. Nothing here touches the prompt or anything stored — it changes the result and the file you download.',
  antiAliasThreshold:
  ...
  presetName:
    'What this set of dial positions is called. Give it the name of the thing it suits rather than the settings it holds — the generator whose sheets need it, or the style of artwork — because the numbers are already on screen and the reason for them is not. Names only have to be different inside one project: a name the chosen project already holds updates that set, while the same name in another project is left alone.',
  presetDescription:
  ...
  spriteCellSource:
    'What every sprite in a pack or a manifest is cut into. Bounding box is the cut this tab has always made: each piece keeps the box the Sprites panel counted and the preview rings, so fifteen pieces off a rig sheet come out at fifteen different sizes. That is what a person slicing artwork by hand wants and what a rig importer cannot take — an importer of cut-out pieces declares one slot size per piece and refuses artwork that does not match it. The other two cut every piece into one fixed cell instead, padding the rest with transparency: Studio target takes the component size the studio’s prompt already states, and Fixed takes the two boxes beside it, for a slot size your own pipeline declares. Nothing is ever squeezed to fit — a sprite larger than the cell is a sheet that came back at a coarser scale than the prompt asked for, and the download is refused and says which piece. It changes only the file you save; the sheet, the previews and the readings stay as they are.',

  spriteCellWidth:
    'How wide each cell is, in drawn pixels — the same units the Sprites panel reports the largest piece in, so the two figures can be read against one another. Type the width your importer declares for the slot these pieces go into, and remember that it is a width at the sheet’s own scale: the Save At control magnifies the cell along with the artwork, so a cell of 20 written at 4× is 80 file pixels across. Every sprite has to fit it.',

  spriteCellHeight:
    'How tall each cell is, in drawn pixels, and the companion to the width beside it. Pieces of one rig rarely want a square cell — a forearm is narrow and long, a pelvis is wide and short — so the two are stated apart rather than as one side. The Save At control magnifies this along with the artwork, and a piece taller than it is refused rather than squeezed.',

  spriteCellAnchorX:
    'Where the artwork sits across its cell, once the cell is wider than the piece drawn in it. Centre is the safe answer for anything symmetric about its own middle. An edge is what a limb wants: a rig registers a piece against the joint it turns on, so a forearm cut into a cell and centred in it swings from the middle of itself and the elbow comes apart in the walk. Whichever you pick is also the horizontal half of the pivot the manifest states, so an importer that reads the pivot lands the piece on the same point you cut it against.',

  spriteCellAnchorY:
    'Where the artwork sits down its cell, and the vertical half of the same statement as the setting beside it. Bottom is where this opens, because a ground-standing sprite is placed by its feet in a three-quarter or top-down game and that is the convention the manifest’s pivot already carried. Top is what a piece hanging from a socket wants — an arm from a shoulder, a lid from a hinge — and middle suits a piece that turns about its own centre, such as a wheel. It is also the vertical half of the pivot the manifest states.',
