 ...
 *
 * It, `SCOPE_AND_PRECEDENCE_HEADING` and `ONE_SIDED_FEATURES_HEADING` are the only headings
 * extracted this way, because they are the only ones another file names. A heading cited solely by
 * the prose around it stays written where it is read. All three sit above the template's own
 * documentation rather than between it and the constant, which would leave that long comment
 * attached to a short string.
 */
