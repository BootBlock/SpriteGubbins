 ...
 *
 * Built once rather than a `find` per render: `TARGET_MODELS` is a compile-time constant, so
 * re-scanning it on every keystroke in the studio is work with no possible change in answer.
 * Shared because two controls ask it — the link beside the target select, and the preview's Copy,
 * open & next — and each needs the same generator site.
 *
