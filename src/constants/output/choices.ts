...
// **Neither of the two share rungs states its range.** Both are a share of the largest component's
// own cell in the grid, and the two percentages are `SHARE_RANGE`'s: a label spelling them out would
// be a hand-written copy of that record, free to drift from the prompt it describes. The field's own
// guidance reads them out of the record instead, and has no width to run out of; what the label owes
// a reader here is which rung is the larger.
