...
// are two ranges per rung, and a label naming one of them misdescribes the prompt on every sheet
// framed the other way. Which of the two a sheet takes is `SheetPlan.scaleUnitFrame`, so it is not
// even fixed within a category: BACKGROUND's parallax set is priced against a cell and its layer
// library against the sheet. The numbers are in the field's own guidance, which has no width to
