 ...
 *
 * **Ordered by what most sheets want**, not by the union's own order: `FIVE_CLASSIC` leads because
 * it is the studio's opening set and the smallest whose drawn views face the camera and turn away
 * from it, and `SINGLE_FRONT` sits below the classic sets rather than first.
 * `CATEGORY_DIRECTION_SETS` lists them the other way round for a different job — its leading entry
 * is the fallback a stored set degrades to — so the two orders are deliberately unrelated and
...
const DIRECTION_SET_CHOICES: readonly OutputChoice<DirectionSet>[] = [
  { value: 'FIVE_CLASSIC', label: 'FIVE_CLASSIC (front, 3/4, side, back-3/4, back)' },
 ...
 * turns to. For seven of the thirteen categories this is the whole list.
 */
