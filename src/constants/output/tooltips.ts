...
export const OUTPUT_TOOLTIPS = {
  ...
  directionalMode:
    'What the sheet has to deliver, and therefore how many components the prompt asks for. The list holds only what this category can actually produce, so it changes with the category above: CORE_DIRECTIONAL_VARIANTS is the usual default where it is offered, while a terrain is laid flat and has no facing to turn, so it offers a tile field instead. CUTOUT_RIG_SINGLE_DIRECTION covers one facing per sheet, so eight directions means eight runs rather than one 120-piece sheet no generator will return in a single pass. The count beside each option is what the whole deliverable asks for — where that is more than one generation the option says so, and each sheet’s own figure is the one its prompt contracts for.',
  surfaceDetail:
