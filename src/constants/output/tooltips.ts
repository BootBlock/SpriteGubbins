...
export const OUTPUT_TOOLTIPS = {
  ...
  paletteLimit:
    ...
    '`STRICT_32_COLOR` and `RESTRAINED_64_COLOR` suit pixel work, and are the only two `RETRO_PIXEL_ART` offers. Painted, cel-shaded and 3D styles usually want `UNRESTRICTED`, because a hard colour count fights the blending they depend on.',
  outlineStyle:
    'How a component’s boundary is drawn where it meets the background. A pixel style draws it one pixel wide, and any other style as a thin line.\n\n' +
    ...
    '`CEL_SHADED` and `HAND_DRAWN_INK` draw their own contour line, so this sets its colour and `OUTLINE_LESS_ALBEDO` is not offered. Below about 32 px a pixel contour takes a serious share of the pixels. On a `PURE_BLACK` background a black outline is asked for as a very dark grey.',
  lightingModel:
    ...
    '`CEL_SHADED`, `RENDERED_3D`, `LOW_POLY_3D` and `CLAY_RENDER` shade from a directional light, so they take `ISOMETRIC_TOP_LEFT` and this control is withdrawn.',
  aspectRatio:
  ...
  renderStyle:
    ...
    'Each states its own surface, so it withdraws Surface Detail Intensity, Palette Limit, Outline System and Lighting & Shading Model: `SILHOUETTE_ONLY` has nothing to light, and `CLAY_RENDER` takes the fixed key light.\n\n' +
