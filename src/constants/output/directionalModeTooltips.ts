 ...
 *
 * **Keyed on the mode alone, so every sentence has to be true of every category and assembly base that
 * offers it.** A pose library is a font’s four glyph sheets, a portrait's expressions and a rigid
 * object's states as much as it is a character’s poses, and a tile field is a nine-slice frame as much
 * as it is a terrain’s two materials meeting — so each entry names the range rather than the one
 * instance its identifier happens to be spelled after. What the chosen pairing actually draws is named by the inventory
