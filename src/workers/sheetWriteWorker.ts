...
import { writeSheet } from '../utils/writeSheet.ts';
 ...
 *
 * Every writer walks each byte of the image several times and then hands the lot to
 * `CompressionStream`, and the tab admits an image of 16.8 million pixels — magnified, since the
 * Save At control offers whatever rung still fits, and again per sprite for the pack, which encodes
 * a PNG apiece. On the main thread that is not slow, it is a **freeze**, and a freeze inside a click
 * handler is the worst kind: React cannot paint the button as pressed, so the first thing the reader
 * sees is the finished file, seconds later. The canvas encoder this replaced was asynchronous and
 * never blocked the handler; this is how that property is kept.
 *
 * **The magnification happens on this thread too**, and that is the same argument rather than a
 * second one. `upscaleNearest` is a loop over the *output*, which the Save At ladder caps at 16.8
 * million pixels — 16.8 million iterations and a 67-megabyte allocation — so on the main thread it
 * is the first half of the freeze, and it would run before the button could be painted as pressed.
 ...
 * same reason and are scaled beside the image, so the two can never be on different coordinates.
 *
 ...
 *
 * **The request *is* the job**, `SheetWriteJob` as `writeSheet` defines it, rather than a message
 * shape of this file's own that would have to be kept in step with it field for field. Which format
 * a press produces is decided there and not here: everything this file does — the guarded reply, the
 * thread's lifetime, the flag the near side sets — is the same whatever is being written, and a
 * branch on the format in a worker is a branch nothing can test without one.
 *
...
self.addEventListener('message', (event: MessageEvent<SheetWriteJob>) => {
 ...
 * reported wrongly if they are merged. The first covers the whole write, where the realistic causes
 * are room — `upscaleNearest` is the single largest allocation in the app, and it is synchronous, so
 * outside a `try` its `RangeError` would escape as an uncaught exception and reach the near side as
 * "the thread could not start", which is not what happened — and a canvas the `.aseprite` format
 * cannot state, which `encodeAseprite` refuses in as many words. The second covers the reply
 * itself. Both report through {@link fail}, which cannot throw — a `failed` post
 * that threw would escape as the very rejection all of this is arranged to prevent.
 */
export async function write(job: SheetWriteJob): Promise<void> {
  let file: WrittenSheet;
  try {
    file = await writeSheet(job);
  } catch (error: unknown) {
    fail(error);
    return;
  }
