...
export interface CustomPaletteIntake {
  /** Every line of the last file or paste that stated no colour. */
...
export function useCustomPaletteIntake(): CustomPaletteIntake {
  ...
  const pin = useCallback(
    (candidate: CustomPalette, read: string) => {
      ...
      if (parsed === null) {
        setProblems([`Nothing in ${read} read as a colour, so the palette is unchanged.`]);
        return;
      }
  ...
  const readText = useCallback(
    (text: string, fallbackName: string, read: string) => {
      ...
      if (reading.entries.length === 0) return;
      ...
      pin({ name: reading.name, entries: reading.entries }, read);
    },
    [pin],
  );
  ...
  return { problems, oversized, acceptFile, acceptPaste, reduceOversized, clear };
}
