...
describe('primaryFacing', () => {
  it('is the resolved run-list facing, whatever any sheet plan does with it', () => {
    // The one function here that reads no mode, and so the one that needs no category: a facing
    // resolved through `output.directions` is the same facing whatever kind of sheet consults it.
    // Every one of its callers gates on `directionSetApplies` first — directly, or through the
    // `splitsIntoFacingRuns` that delegates to it — which is where the category decides whether this
    // answer is worth anything at all.
