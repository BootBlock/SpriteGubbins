 ...
 *
 * The step profile's evidence says how strongly every column and row reads as a boundary — its
 * magnitude on a resampled axis, and its transitions on a crisp one, as `stepProfile` argues; this
 * reads the positions carrying *far more of it than the axis's own background* as boundary
 * candidates, and merges the candidates a softened ramp spreads across neighbouring positions into
 * one line each. It is the
