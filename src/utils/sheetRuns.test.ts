...
describe('sheetRuns', () => {
  ...
  it('counts a subject’s additional anatomy once per facing, as the prompts do', () => {
    ...
    const total = batchComponentCount('CHARACTER', subject, runs, anatomy);
    expect(total).toBe(runs.reduce((sum, run) => sum + statedCount(run.promptText), 0));
    expect(total).toBe(batchComponentCount('CHARACTER', subject, runs, []) + 3 * runs.length);
  });
  ...
  it('sums the sheets of a series, which do not cost the same', () => {
    ...
    expect(batchComponentCount('CHARACTER', SUBJECT, runs, [])).toBe(
