 ...
 * sixty-four reads per position per table, eleven tables deep, once per candidate.
 *
 * `Float64Array` throughout, because the likeness score's tables accumulate squares — four times
 * 255² times the pixel count, which is past where a 32-bit float still counts exactly.
 *
...
/** The summed-area table over `plane`. */
export function integralImage(plane: Float64Array, width: number, height: number): Float64Array {
  const stride = width + 1;
  const table = new Float64Array(stride * (height + 1));
  for (let y = 0; y < height; y += 1) {
    let row = 0;
    for (let x = 0; x < width; x += 1) {
      row += plane[y * width + x] ?? 0;
