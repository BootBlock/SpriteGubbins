 ...
 * by is provenance — the subject it was loaded for — and provenance is exactly what a resolver
 * called once per change is placed to lose.
 */
describe('a rig contract across a change of subject', () => {
  ...
  it('does not follow the reader into another category, though the rig mode does', () => {
    ...
    const resolved = resolveOutputForSubject('CREATURE', defaultSubjectFor('CREATURE'), rigged, 'CHARACTER');

    expect(resolved.rigMode).toBe('CUTOUT_RIG');
    expect(resolved.rigContract).toBeNull();
  });

  it('survives a change that stays inside the category it was loaded under', () => {
    ...
    const resolved = resolveOutputForSubject(
      'CHARACTER',
  ...
  it('goes when the assembly base leaves the subject with no rig at all', () => {
    ...
    const resolved = resolveOutputForSubject('OBJECT', rigid, rigged, 'OBJECT');
