...
describe('generatePrompt — a term the prompt uses is a term the prompt defines', () => {
  ...
  it('never names a native pixel without carrying the block that defines one', () => {
    ...
    // heading — so every target is swept rather than trusting the body alone. Both emit flags stay
    // on throughout, because a section that only appears when one is set is prose no other loop
    // here reaches, and a term named inside one would be invisible to this check.
    const EMITTING = { emitComponentMap: true, emitPromptFeedback: true } as const;
    for (const targetModel of TARGET_MODEL_IDS) {
      for (const renderStyle of RENDER_STYLES) {
        check(
          generatePrompt('CHARACTER', SUBJECT, withOutput({ ...EMITTING, targetModel, renderStyle })),
