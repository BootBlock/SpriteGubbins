...
import type { SpriteDecision, SpriteEdit } from '../types/spriteAssignment.ts';
import { pinnedSprite } from './spritePin.ts';
 ...
 *
 * **A decision is matched to a sprite by containment, not by position** — see `spritePin.ts`, which
 * is where that choice is argued. Everything unmatched is counted into `lost` and dropped: a join
 * whose partner has gone, a decision whose sprite has gone, and the second of two decisions that a
 * merge has landed on one sprite.
 *
...
export function shapeSheet(boxes: readonly SpriteBox[], edits: readonly SpriteEdit[]): ShapedSheet {
  ...
  for (const edit of edits) {
    const index = pinnedSprite(boxes, edit.pin);
    // A second decision on one sprite is the first one's sprite having been merged into it. The
    // earlier decision keeps the sprite, because it is the one the panel and the preview have been
    // showing; the later is counted lost rather than overwriting it without a word.
    if (index === null || decisions[index] !== null) {
      lost += 1;
      continue;
    }
  ...
  for (const [index, decision] of decisions.entries()) {
    ...
    const partner = pinnedSprite(boxes, decision.to);
  ...
  return buildPieces(boxes, decisions, groups, lost);
}
...
function buildPieces(
  ...
  return { pieces, pieceOf, decisions, lost };
}
