 ...
 * Nothing is lost by the order either: `keyBackground` writes only full transparency or the pixel
 * it was handed at its own alpha — the despill changes a pixel's colour and never its alpha — so
 * there is no partial alpha of its own for this to threshold, and the coverage it reads is always
 * the sheet's own. The hardening then goes ahead of the mesh so the step profile weighs a hard
 * boundary rather than a ramp, which is the same reason the key goes ahead of it.
 *
