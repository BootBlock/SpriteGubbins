 ...
 *
 * Counts, and takes the heaviest class, where `bestPhase` in `bestPhase.ts` takes the centre of a
 * resampled axis's magnitude, because this is the exact question and that is the soft-edged one: a
 * softened ramp is three transitions spread about one step. Ties go to the
