 ...
 * result; painted here it is one pass over an image `grid²` times smaller than the sheet it came
 * from, on a value the preview is holding anyway.
 *
...
export function heatmapImage(map: DifferenceMap, scale: number): ImageData {
  ...
  for (let cell = 0; cell < map.cells.length; cell += 1) {
    ...
    image.data.set(RAMP.subarray(from, from + CHANNELS_PER_PIXEL), cell * CHANNELS_PER_PIXEL);
  }
