...
const MOVED: { readonly [K in keyof QuantiseTuning]: (from: QuantiseSettings) => QuantiseSettings } = {
  ...
  antiAlias: (from) => ({ ...from, antiAlias: 'BOTH' }),
  antiAliasThreshold: (from) => ({ ...from, antiAliasThreshold: 48 }),
  antiAliasStrength: (from) => ({ ...from, antiAliasStrength: 50 }),
  antiAliasRun: (from) => ({ ...from, antiAliasRun: 5 }),
  antiAliasPalette: (from) => ({ ...from, antiAliasPalette: 'BLEND' }),
};
