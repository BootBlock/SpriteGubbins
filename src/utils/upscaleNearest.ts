 ...
 *
 * Two callers, one on each side of the app's boundary. The encoder's worker magnifies a quantised
 * result so a 16-pixel sprite can be *seen* in the file it ships in — the saved sheet stays exact,
 * since a block of identical pixels downsamples back to the pixel it came from, and it happens
 * there rather than at the button because this loop is over the *output* and so is the expensive
 * half of a magnified download. And the pixel-reader tests build their fixtures with it, because
 * "art drawn at `g`" *is* an image whose every pixel is a `g × g` block — what a model returns when
 * it draws pixel art at 16 × 16 and hands back a 128 × 128 sheet.
 *
