...
/**
 * Every sheet the studio can reach, which is the set both counts have to agree over: each category's
 * assembly bases, modes, direction sets and sheets.
 *
 * **Each base with the subject that selects it**, because a declared base draws sheets of its own (issue
 * #283) and a rigid object's views are named like any other. The `clothing` stays the standard subject's
 * empty value rather than the default's, because BACKGROUND's default declines its overlays and would
