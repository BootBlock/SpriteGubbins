 ...
 *
 * The measurement two different questions are answered from, which is why it has a module of its
 * own: `estimatePixelGrid` reads it for the *spacing* the change repeats at, and `bestGridOffset`
 * reads it for *where* a known spacing sits against the image. Both walk the same totals, and two
 * implementations of the walk would eventually disagree about the same sheet.
 *
