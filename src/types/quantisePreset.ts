 ...
 * already names the thirteen dials `quantiseImage` is handed; the tab has three more — the two keying
 * settings, which reach the pipeline as a `BackgroundKeying` rather than as fields, and the palette
 * snap, which reaches it inside a `ColorReduction`. Listing those thirteen again here would be two lists
 ...
 *
 * **The whole set is workflow intent**, which is why `useQuantiseStore` carries all sixteen across
