 ...
 * already names the dials `quantiseImage` is handed; the tab has three more — the two keying
 * settings, which reach the pipeline as a `BackgroundKeying` rather than as fields, and the palette
 * snap, which reaches it inside a `ColorReduction`. Listing the pipeline's dials again here would be
 * two lists of the same dials free to drift, and a dial added to the pipeline's set — which is where a new
 ...
 *
 * **The whole set is workflow intent**, which is why `useQuantiseStore` carries every one across
